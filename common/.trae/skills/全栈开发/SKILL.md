...
name: 全栈开发
description: 进行全栈开发时触发，包括新功能开发、功能修改、Bug 修复、代码重构等场景，提供从需求分析到提交部署的完整 7 步流程指导
--------------------------------------------------------------------------
...
```
┌─────────────────────────────────────────────────────────┐
│ 1. HTTP POST /api/admin//user                        │
│    Body: {"username": "admin", "email": "admin@ix.com"}│
└─────────────────────────────────────────────────────────┘
                        ↓
┌─────────────────────────────────────────────────────────┐
│ 2. API Handler (internal/app/base/api/user.go)         │
...
```
┌─────────────────────────────────────────────────────────┐
│ 1. HTTP GET /api/admin//user/info                    │
...
|---------|------|---------|---------|-------------|
| **HTTP → DTO** | API Handler | JSON → Request DTO | `ctx.ShouldBindJSON()` | [`user.go:59`](d:\g\ixpay-pro\server\internal\app\base\api\user.go#L59) |
| **DTO → 参数** | API Handler | Request DTO → 简单参数 | 直接传递字段 | [`user.go:66`](d:\g\ixpay-pro\server\internal\app\base\api\user.go#L66) |
| **参数 → Domain Entity** | Domain Service | 简单参数 → Entity | 构造器创建 | [`user_service.go:132`](d:\g\ixpay-pro\server\internal\domain\base\service\user_service.go#L132) |
| **Domain Entity → Database Model** ⭐ | Repository | Entity → Model | [`fromDomain()`](d:\g\ixpay-pro\server\internal\persistence\base\user_repository.go#L64-L99) | [`user_repository.go:64`](d:\g\ixpay-pro\server\internal\persistence\base\user_repository.go#L64) |
| **Database Model → Domain Entity** ⭐ | Repository | Model → Entity | [`toDomain()`](d:\g\ixpay-pro\server\internal\persistence\base\user_repository.go#L38-L62) | [`user_repository.go:38`](d:\g\ixpay-pro\server\internal\persistence\base\user_repository.go#L38) |
| **Domain Entity → DTO** | API Handler | Entity → Response DTO | 手动构建 | [`user.go:148`](d:\g\ixpay-pro\server\internal\app\base\api\user.go#L148) |
...
func (m *userModel) toDomain() *entity.User {
    return &entity.User{
        ID:           strconv.FormatInt(m.ID, 10),      // ✅ int64 → string
        Username:     m.Username,
        PositionID:   strconv.FormatInt(m.PositionID, 10),  // ✅ int64 → string
        DepartmentID: strconv.FormatInt(m.DepartmentID, 10), // ✅ int64 → string
        CreatedBy:    strconv.FormatInt(m.CreatedBy, 10),    // ✅ int64 → string
    }
}

// ⭐ Domain Entity → Database Model（写入时使用）
func fromDomain(user *entity.User) (*userModel, error) {
    id, _ := strconv.ParseInt(user.ID, 10, 64)         // ✅ string → int64
    positionID, _ := strconv.ParseInt(user.PositionID, 10, 64)
...
```go
// internal/app/base/api/user.go

// ⭐ 单条查询：手动构建 Response DTO
func (c *UserController) GetUserInfo(ctx *gin.Context) {
    // 1️⃣ 调用 Domain Service，返回 Domain Entity
    user, err := c.service.GetUserInfo(userID.(int64))
...
type userModel struct {
    ID int64 `gorm:"primaryKey"`  // ✅ 数据库存储 int64
}
...
type UserResponse struct {
    ID string `json:"id,string"`  // ✅ DTO 使用 string
}
```
...
// Database Model（有 GORM 标签，ID 为 int64）
type userModel struct {
    ID        int64          `gorm:"primaryKey"`  // ✅ 数据库存储 int64
    ...
    CreatedAt time.Time
    UpdatedAt time.Time
    DeletedAt gorm.DeletedAt `gorm:"index"`  // ✅ GORM 标准软删除类型
}
...
func fromDomain(user *entity.User) (*userModel, error) {
    ...
    if err != nil {
        return nil, fmt.Errorf("ID 格式错误：%w", err)
    }
    return &userModel{
        ID:       id,
        Username: user.Username,
...
// 示例：
//    // 数据库 Model（使用 gorm.DeletedAt）
//    type User struct {
//        ID        uint
//        Username  string
//        DeletedAt gorm.DeletedAt `gorm:"index"`
//    }
//    // API Response DTO（不包含 DeletedAt）
//    type UserResponse struct {
//        ID       string `json:"id"`
//        Username string `json:"username"`
//    }
...
**API 设计**:
1. GET    /api/admin//user/list  - 用户列表
2. POST   /api/admin//user       - 创建用户
3. PUT    /api/admin//user/:id   - 更新用户
4. DELETE /api/admin//user/:id   - 删除用户
5. GET    /api/admin//user/:id   - 用户详情
