...
| | GORM | v1.31.1 | 功能丰富的 ORM 库，简化数据库操作 |
| **缓存** | Redis (go-redis) | v9.17.2 | 高性能键值存储，用于缓存和会话管理 |
...
| **系统监控** | gopsutil | v4.26.2 | 系统资源监控库，获取 CPU、内存、磁盘等信息 |
| **监控** | Prometheus | v1.23.2 | 开源监控系统，用于系统性能监控 |
| **限流** | golang.org/x/time/rate | v0.14.0 | API 速率限制库，防止系统过载 |
| **熔断器** | gobreaker | v1.0.0 | 服务熔断库，提高系统容错能力 |
...
| 类别 | 技术/框架 | 版本 | 说明 |
|------|-----------|------|------|
| **开发框架** | Vue 3 | v3.5.18 | 现代化前端框架，提供响应式数据绑定和组件化开发 |
| **UI 组件库** | Element Plus | v2.11.2 | 基于 Vue 3 的 UI 组件库，提供丰富的界面元素 |
| **开发语言** | TypeScript | v5.8.0 | 静态类型检查，提高代码质量和可维护性 |
| **构建工具** | Vite | v7.0.6 | 现代前端构建工具，提供快速的开发体验 |
| **状态管理** | Pinia | v3.0.3 | Vue 3 官方推荐的状态管理库 |
| **路由** | Vue Router | v4.5.1 | Vue 官方路由库，实现单页应用导航 |
| **HTTP 客户端** | Axios | v1.12.2 | 基于 Promise 的 HTTP 客户端，用于 API 调用 |
| **图表库** | ECharts | v6.0.0 | 数据可视化图表库 |
| **CSS 框架** | Tailwind CSS | v3.4.17 | 实用优先的 CSS 框架 |
| **代码检查** | ESLint | v9.31.0 | JavaScript/TypeScript 代码检查工具 |
| **代码格式化** | Prettier | v3.6.2 | 代码格式化工具 |
...
- **🏢 部门管理**: 组织架构的部门管理，支持树形结构和层级关系
- **📌 岗位管理**: 岗位信息的增删改查，支持用户岗位关联
- **📢 通知公告**: 系统通知公告的发布和管理，支持阅读记录追踪
- **📝 操作日志**: 记录用户操作日志，支持日志查询和分析
- **🔑 登录日志**: 记录用户登录日志，支持登录行为分析
...
- **📈 系统监控**: 实时监控系统 CPU、内存、磁盘等系统资源使用情况
- **👥 在线用户**: 实时在线用户监控和管理
- **🔄 熔断保护**: 集成 gobreaker 熔断器，提高系统容错能力
