// Package docs Code generated by swaggo/swag. DO NOT EDIT
package docs

import "github.com/swaggo/swag"

const docTemplate = `{
    "schemes": {{ marshal .Schemes }},
    "swagger": "2.0",
    "info": {
        "description": "{{escape .Description}}",
        "title": "{{.Title}}",
        "termsOfService": "http://swagger.io/terms/",
        "contact": {
            "name": "API Support",
            "email": "support@example.com"
        },
        "license": {
            "name": "Apache 2.0",
            "url": "http://www.apache.org/licenses/LICENSE-2.0.html"
        },
        "version": "{{.Version}}"
    },
    "host": "{{.Host}}",
    "basePath": "{{.BasePath}}",
    "paths": {
        "/api//pay/notify/wechat": {
            "post": {
                "description": "微信支付回调接口，用于处理支付结果通知",
                "consumes": [
                    "application/x-www-form-urlencoded"
                ],
                "produces": [
                    "text/xml"
                ],
                "tags": [
                    "支付管理"
                ],
                "summary": "微信支付通知",
                "responses": {
                    "200": {
                        "description": "成功响应",
                        "schema": {
                            "type": "string"
                        }
                    }
                }
            }
        },
        "/api//payment": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取当前登录用户的所有支付记录",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "支付管理"
                ],
                "summary": "获取用户支付列表",
                "responses": {
                    "200": {
                        "description": "支付列表及分页信息",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api//payment/{id}": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据 ID 查询支付详情",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "支付管理"
                ],
                "summary": "查询支付",
                "parameters": [
                    {
                        "type": "string",
                        "description": "支付 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "支付详情",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_wx_response.PaymentResponse"
                            }
                        }
                    },
                    "400": {
                        "description": "无效的支付 ID",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api//payment/{id}/cancel": {
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据 ID 取消一笔支付",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "支付管理"
                ],
                "summary": "取消支付",
                "parameters": [
                    {
                        "type": "string",
                        "description": "支付 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "取消后的支付详情",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_wx_response.PaymentResponse"
                            }
                        }
                    },
                    "400": {
                        "description": "无效的支付 ID",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/apis": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取系统中注册的所有 API 路由信息（管理员权限），支持分页和搜索",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "API 路由管理"
                ],
                "summary": "获取所有 API 路由列表",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "页码，默认为 1",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "每页数量，默认为 100",
                        "name": "pageSize",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "搜索关键词（支持路径、描述、分组）",
                        "name": "keyword",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "路由分组",
                        "name": "group",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "路由列表",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.PageResult"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "创建新的 API 路由信息（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "API 路由管理"
                ],
                "summary": "创建 API 路由",
                "parameters": [
                    {
                        "description": "API 路由信息",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.CreateAPIRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "创建成功的路由信息",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.APIResponse"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "参数错误",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/api/admin/apis/:id": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据 ID 获取指定的 API 路由信息（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "API 路由管理"
                ],
                "summary": "根据 ID 获取 API 路由",
                "parameters": [
                    {
                        "type": "string",
                        "description": "路由 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "路由信息",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.APIResponse"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "404": {
                        "description": "路由不存在",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "更新指定的 API 路由信息（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "API 路由管理"
                ],
                "summary": "更新 API 路由",
                "parameters": [
                    {
                        "type": "string",
                        "description": "路由 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "更新的路由信息",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.UpdateAPIRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "更新成功的路由信息",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.APIResponse"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "参数错误",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "404": {
                        "description": "路由不存在",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "删除指定的 API 路由信息（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "API 路由管理"
                ],
                "summary": "删除 API 路由",
                "parameters": [
                    {
                        "type": "string",
                        "description": "路由 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "删除成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "参数错误",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "404": {
                        "description": "路由不存在",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/api/admin/apis/list": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "分页获取 API 路由列表，支持过滤（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "API 路由管理"
                ],
                "summary": "获取 API 路由列表",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "页码，默认为 1",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "每页数量，默认为 10",
                        "name": "pageSize",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "路由分组",
                        "name": "group",
                        "in": "query"
                    },
                    {
                        "type": "boolean",
                        "description": "是否需要认证",
                        "name": "authRequired",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "路由列表",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.PageResult"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/api/admin/auth/captcha": {
            "post": {
                "description": "获取登录验证码",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "认证服务"
                ],
                "summary": "获取验证码",
                "responses": {
                    "200": {
                        "description": "验证码信息",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.CaptchaResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/auth/login": {
            "post": {
                "description": "用户登录获取访问令牌",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "认证服务"
                ],
                "summary": "用户登录",
                "parameters": [
                    {
                        "description": "登录请求参数",
                        "name": "login",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.LoginRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "登录成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.LoginResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "用户名或密码错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/auth/logout": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "用户登出系统",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "认证服务"
                ],
                "summary": "用户登出",
                "responses": {
                    "200": {
                        "description": "登出成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/auth/refresh-token": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "使用刷新令牌获取新的访问令牌",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "认证服务"
                ],
                "summary": "刷新令牌",
                "responses": {
                    "200": {
                        "description": "刷新成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.LoginResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "刷新令牌无效",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/auth/register": {
            "post": {
                "description": "创建新用户账户",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "基础服务"
                ],
                "summary": "用户注册",
                "parameters": [
                    {
                        "description": "注册请求参数",
                        "name": "register",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.RegisterRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "注册成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.User"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/btn-perms": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取按钮权限列表",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "按钮权限管理"
                ],
                "summary": "获取按钮权限列表",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "pageSize",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "format": "int64",
                        "description": "菜单 ID",
                        "name": "menu_id",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "按钮编码",
                        "name": "code",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "按钮名称",
                        "name": "name",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "状态(-1:全部,0:禁用,1:启用)",
                        "name": "status",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "allOf": [
                                                {
                                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.PageResult"
                                                },
                                                {
                                                    "type": "object",
                                                    "properties": {
                                                        "list": {
                                                            "type": "array",
                                                            "items": {
                                                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.BtnPerm"
                                                            }
                                                        },
                                                        "page": {
                                                            "type": "integer"
                                                        },
                                                        "pageSize": {
                                                            "type": "integer"
                                                        },
                                                        "total": {
                                                            "type": "integer",
                                                            "format": "int64"
                                                        }
                                                    }
                                                }
                                            ]
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "更新按钮权限信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "按钮权限管理"
                ],
                "summary": "更新按钮权限",
                "parameters": [
                    {
                        "description": "按钮权限信息",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.UpdateBtnPermRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "创建新的按钮权限",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "按钮权限管理"
                ],
                "summary": "创建按钮权限",
                "parameters": [
                    {
                        "description": "按钮权限信息",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.CreateBtnPermRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "删除按钮权限",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "按钮权限管理"
                ],
                "summary": "删除按钮权限",
                "parameters": [
                    {
                        "description": "按钮权限ID",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.DeleteBtnPermRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                        }
                    }
                }
            }
        },
        "/api/admin/btn-perms/api-routes": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取按钮权限关联的 API 路由",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "按钮权限管理"
                ],
                "summary": "获取按钮权限关联的 API 路由",
                "parameters": [
                    {
                        "type": "integer",
                        "format": "int64",
                        "description": "按钮权限 ID",
                        "name": "btnPermId",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.API"
                                            }
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/api/admin/btn-perms/assign-api-routes": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "为按钮权限分配 API 路由",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "按钮权限管理"
                ],
                "summary": "为按钮权限分配 API 路由",
                "parameters": [
                    {
                        "description": "按钮 ID 和 API 路由 ID 列表",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.AssignToBtnPermRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                        }
                    }
                }
            }
        },
        "/api/admin/btn-perms/assign-to-role": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "为角色分配按钮权限",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "按钮权限管理"
                ],
                "summary": "为角色分配按钮权限",
                "parameters": [
                    {
                        "description": "角色 ID 和按钮权限 ID 列表",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.AssignBtnPermToRoleRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                        }
                    }
                }
            }
        },
        "/api/admin/btn-perms/by-menu": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取菜单下的所有按钮权限",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "按钮权限管理"
                ],
                "summary": "获取菜单下的按钮权限",
                "parameters": [
                    {
                        "type": "integer",
                        "format": "int64",
                        "description": "菜单 ID",
                        "name": "menuId",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.BtnPerm"
                                            }
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/api/admin/btn-perms/by-role": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取角色拥有的按钮权限",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "按钮权限管理"
                ],
                "summary": "获取角色拥有的按钮权限",
                "parameters": [
                    {
                        "type": "integer",
                        "format": "int64",
                        "description": "角色 ID",
                        "name": "roleId",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.BtnPermForRole"
                                            }
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/api/admin/btn-perms/detail": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据ID获取按钮权限详情",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "按钮权限管理"
                ],
                "summary": "根据ID获取按钮权限",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "按钮权限ID",
                        "name": "id",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.BtnPermResponse"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/api/admin/btn-perms/for-route": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取 API 路由关联的按钮权限",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "按钮权限管理"
                ],
                "summary": "获取 API 路由关联的按钮权限",
                "parameters": [
                    {
                        "type": "integer",
                        "format": "int64",
                        "description": "API 路由 ID",
                        "name": "routeId",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.BtnPerm"
                                            }
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/api/admin/btn-perms/revoke-api-route": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "从按钮权限撤销API路由",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "按钮权限管理"
                ],
                "summary": "从按钮权限撤销API路由",
                "parameters": [
                    {
                        "description": "按钮ID和API路由ID",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.RevokeFromBtnPermRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                        }
                    }
                }
            }
        },
        "/api/admin/btn-perms/revoke-from-role": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "从角色撤销按钮权限",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "按钮权限管理"
                ],
                "summary": "从角色撤销按钮权限",
                "parameters": [
                    {
                        "description": "角色ID和按钮权限ID",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.RevokeBtnPermFromRoleRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                        }
                    }
                }
            }
        },
        "/api/admin/config": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取系统配置列表",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "系统配置管理"
                ],
                "summary": "获取配置列表",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "每页条数",
                        "name": "pageSize",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "配置键",
                        "name": "config_key",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "状态：1-启用 0-禁用",
                        "name": "status",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "配置列表",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.ConfigListResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "更新系统配置",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "系统配置管理"
                ],
                "summary": "更新配置",
                "parameters": [
                    {
                        "description": "更新配置请求参数",
                        "name": "config",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.UpdateConfigRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "更新成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "创建新的系统配置",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "系统配置管理"
                ],
                "summary": "创建配置",
                "parameters": [
                    {
                        "description": "创建配置请求参数",
                        "name": "config",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.CreateConfigRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "创建成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.ConfigResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/config/:id": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据ID获取配置详情",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "系统配置管理"
                ],
                "summary": "根据ID获取配置",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "配置ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "配置详情",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.ConfigResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "删除系统配置",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "系统配置管理"
                ],
                "summary": "删除配置",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "配置ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "删除成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/config/active": {
            "get": {
                "description": "获取所有启用的系统配置",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "系统配置管理"
                ],
                "summary": "获取所有启用的配置",
                "responses": {
                    "200": {
                        "description": "配置列表",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.ConfigResponse"
                                            }
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/config/key": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据配置键获取配置详情",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "系统配置管理"
                ],
                "summary": "根据配置键获取配置",
                "parameters": [
                    {
                        "type": "string",
                        "description": "配置键",
                        "name": "config_key",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "配置详情",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.ConfigResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/dept": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取部门树形结构列表（支持筛选）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "部门管理"
                ],
                "summary": "获取部门列表",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "page_size",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "父部门 ID",
                        "name": "parent_id",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "状态 (0:禁用，1:启用)",
                        "name": "status",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "部门列表",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.DepartmentListResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "更新部门信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "部门管理"
                ],
                "summary": "更新部门",
                "parameters": [
                    {
                        "description": "部门信息",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.UpdateDepartmentRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "更新成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.DepartmentResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "创建新的部门",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "部门管理"
                ],
                "summary": "创建部门",
                "parameters": [
                    {
                        "description": "部门信息",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.CreateDepartmentRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "创建成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.DepartmentResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/dept/:id": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据 ID 获取部门详细信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "部门管理"
                ],
                "summary": "获取部门详情",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "部门 ID",
                        "name": "id",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "部门详情",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.DepartmentResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "删除部门（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "部门管理"
                ],
                "summary": "删除部门",
                "parameters": [
                    {
                        "type": "string",
                        "description": "部门 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "删除成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/dept/:id/leader": {
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "更新部门负责人",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "部门管理"
                ],
                "summary": "更新部门负责人",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "部门 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "部门负责人信息",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.UpdateDepartmentLeaderRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "更新成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/dept/tree": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取完整的部门树形结构数据",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "部门管理"
                ],
                "summary": "获取部门树形结构",
                "responses": {
                    "200": {
                        "description": "部门树",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.Department"
                                            }
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/dict": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取字典列表",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "字典管理"
                ],
                "summary": "获取字典列表",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "每页条数",
                        "name": "pageSize",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "字典编码",
                        "name": "dict_code",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "字典名称",
                        "name": "dict_name",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "状态：1-启用 0-禁用",
                        "name": "status",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "字典列表",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.DictListResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "更新字典信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "字典管理"
                ],
                "summary": "更新字典",
                "parameters": [
                    {
                        "description": "更新字典请求参数",
                        "name": "dict",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.UpdateDictRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "更新成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "创建新的字典",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "字典管理"
                ],
                "summary": "创建字典",
                "parameters": [
                    {
                        "description": "创建字典请求参数",
                        "name": "dict",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.CreateDictRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "创建成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.DictResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/dict/:id": {
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "删除字典（会同时删除字典下的所有字典项）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "字典管理"
                ],
                "summary": "删除字典",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "字典ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "删除成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/dict/code/{code}/active-items": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据字典编码获取该字典下所有启用的字典项，供前端下拉使用",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "字典管理"
                ],
                "summary": "根据字典编码获取启用的字典项",
                "parameters": [
                    {
                        "type": "string",
                        "description": "字典编码",
                        "name": "code",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "字典项列表",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.DictItemResponse"
                                            }
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/dict/item": {
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "更新字典项信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "字典管理"
                ],
                "summary": "更新字典项",
                "parameters": [
                    {
                        "description": "更新字典项请求参数",
                        "name": "dict_item",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.UpdateDictItemRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "更新成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "创建新的字典项",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "字典管理"
                ],
                "summary": "创建字典项",
                "parameters": [
                    {
                        "description": "创建字典项请求参数",
                        "name": "dict_item",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.CreateDictItemRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "创建成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.DictItemResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/dict/item/:id": {
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "删除字典项",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "字典管理"
                ],
                "summary": "删除字典项",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "字典项ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "删除成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/dict/items": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据字典ID获取该字典下的所有字典项",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "字典管理"
                ],
                "summary": "根据字典ID获取字典项列表",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "字典ID",
                        "name": "dict_id",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "字典项列表",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.DictItemListResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/event": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取事件列表（支持分页和筛选）（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "事件管理"
                ],
                "summary": "获取事件列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "事件名称",
                        "name": "name",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "事件状态",
                        "name": "status",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "优先级",
                        "name": "priority",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "开始日期",
                        "name": "startDate",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "结束日期",
                        "name": "endDate",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "pageSize",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "事件列表",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.EventListResponse"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "发布一个新的事件（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "事件管理"
                ],
                "summary": "创建事件",
                "parameters": [
                    {
                        "description": "创建事件请求参数",
                        "name": "event",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.CreateEventRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "事件创建成功",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.EventResponse"
                            }
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/event/:id": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据 ID 获取事件详情（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "事件管理"
                ],
                "summary": "获取事件详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "事件 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "事件详情",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.EventResponse"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "事件不存在",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/event/:id/retry": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "重试一个失败的事件（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "事件管理"
                ],
                "summary": "重试事件",
                "parameters": [
                    {
                        "type": "string",
                        "description": "事件 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "事件重试成功",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "事件不存在",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/event/dead-letters": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取死信队列中的事件列表（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "事件管理"
                ],
                "summary": "获取死信列表",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "pageSize",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "死信列表",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.DeadLetterListResponse"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/event/dead-letters/:id/retry": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "重试死信队列中的事件（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "事件管理"
                ],
                "summary": "重试死信",
                "parameters": [
                    {
                        "type": "string",
                        "description": "死信 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "死信重试成功",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "死信不存在",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/jwt/jsonInBlacklist": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "将 JWT token 加入黑名单，使其立即失效",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "认证服务"
                ],
                "summary": "将 JWT token 加入黑名单",
                "parameters": [
                    {
                        "description": "JWT token",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.JsonInBlacklistRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "加入黑名单成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/login-log": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取登录日志列表（支持分页和筛选）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "登录日志管理"
                ],
                "summary": "获取登录日志列表",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "page_size",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "format": "int64",
                        "description": "用户 ID",
                        "name": "user_id",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "用户名",
                        "name": "userName",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "登录 IP",
                        "name": "login_ip",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "登录结果：0-失败，1-成功",
                        "name": "result",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "开始日期（YYYY-MM-DD）",
                        "name": "start_date",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "结束日期（YYYY-MM-DD）",
                        "name": "end_date",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "登录日志列表",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.LoginLogListResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "记录用户登录日志（仅供内部调用）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "登录日志管理"
                ],
                "summary": "记录登录日志（内部）",
                "parameters": [
                    {
                        "description": "登录日志信息",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.RecordLoginRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "记录成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/login-log/:id": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据 ID 获取登录日志详细信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "登录日志管理"
                ],
                "summary": "获取登录日志详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "登录日志 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "登录日志详情",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.LoginLogDetailResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/login-log/abnormal": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取异常登录记录（同一 IP 多次失败、异地登录等）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "登录日志管理"
                ],
                "summary": "获取异常登录查询",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "page_size",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "异常登录列表",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "allOf": [
                                                {
                                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.PageResult"
                                                },
                                                {
                                                    "type": "object",
                                                    "properties": {
                                                        "list": {
                                                            "type": "array",
                                                            "items": {
                                                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.AbnormalLoginInfoDTO"
                                                            }
                                                        }
                                                    }
                                                }
                                            ]
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/login-log/batch-delete": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "批量删除指定的登录日志记录",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "登录日志管理"
                ],
                "summary": "批量删除登录日志",
                "parameters": [
                    {
                        "description": "批量删除请求参数",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.BatchDeleteLoginLogsRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "删除成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/login-log/clear": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据时间范围清空登录日志",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "登录日志管理"
                ],
                "summary": "清空登录日志",
                "parameters": [
                    {
                        "description": "清空请求参数",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.ClearLoginLogsRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "清空成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/login-log/statistics": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取登录统计信息（按日期、用户、状态等）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "登录日志管理"
                ],
                "summary": "获取登录统计",
                "parameters": [
                    {
                        "type": "string",
                        "description": "开始日期（YYYY-MM-DD）",
                        "name": "start_date",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "结束日期（YYYY-MM-DD）",
                        "name": "end_date",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "统计信息",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.LoginStatistics"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/logs": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取系统操作日志列表，支持分页和多条件过滤",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "系统管理"
                ],
                "summary": "获取操作日志列表",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "pageSize",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "开始时间 (格式:2006-01-02)",
                        "name": "startTime",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "结束时间 (格式:2006-01-02)",
                        "name": "endTime",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "用户名",
                        "name": "userName",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "操作模块",
                        "name": "module",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "操作类型",
                        "name": "operationType",
                        "in": "query"
                    },
                    {
                        "type": "boolean",
                        "description": "操作结果",
                        "name": "isSuccess",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "操作日志列表",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.OperationLogListResponse"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/logs/:id": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据日志 ID 获取详细的操作日志信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "系统管理"
                ],
                "summary": "根据 ID 获取操作日志",
                "parameters": [
                    {
                        "type": "string",
                        "description": "日志 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "操作日志详情",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.OperationLogResponse"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "日志不存在",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据日志ID删除指定的操作日志",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "系统管理"
                ],
                "summary": "根据ID删除操作日志",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "日志ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "删除成功",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/logs/batch-delete": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "批量删除指定 ID 列表的操作日志",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "系统管理"
                ],
                "summary": "批量删除操作日志",
                "parameters": [
                    {
                        "description": "批量删除请求参数",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.BatchDeleteOperationLogRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "批量删除成功",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/logs/clear": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "清空指定时间范围的操作日志",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "系统管理"
                ],
                "summary": "根据时间范围清空操作日志",
                "parameters": [
                    {
                        "description": "清空日志请求参数",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.ClearOperationLogByTimeRangeRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "清空成功",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/logs/statistics": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取操作日志的统计信息，包括操作类型分布等",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "系统管理"
                ],
                "summary": "获取操作日志统计信息",
                "parameters": [
                    {
                        "type": "string",
                        "description": "开始时间 (格式:2006-01-02)",
                        "name": "startTime",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "结束时间 (格式:2006-01-02)",
                        "name": "endTime",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "操作日志统计信息",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.OperationLogStatisticsResponse"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/menu": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取系统菜单列表",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "菜单管理"
                ],
                "summary": "获取菜单列表",
                "responses": {
                    "200": {
                        "description": "菜单列表",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.Menu"
                                            }
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/menu/:id": {
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "删除菜单（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "菜单管理"
                ],
                "summary": "删除菜单",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "菜单ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "删除成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/menu/:id/delete-impact": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取删除菜单前的影响范围评估（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "菜单管理"
                ],
                "summary": "获取菜单删除影响评估",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "菜单 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "影响评估结果",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.DeleteImpact"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "菜单不存在",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/menu/page": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取菜单分页列表（支持过滤）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "菜单管理"
                ],
                "summary": "获取菜单分页列表",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "page_size",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "菜单标题",
                        "name": "title",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "状态",
                        "name": "status",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "菜单分页列表",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "allOf": [
                                                {
                                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.PageResult"
                                                },
                                                {
                                                    "type": "object",
                                                    "properties": {
                                                        "list": {
                                                            "type": "array",
                                                            "items": {
                                                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.MenuResponse"
                                                            }
                                                        }
                                                    }
                                                }
                                            ]
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/menu/tree": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取完整的菜单树结构（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "菜单管理"
                ],
                "summary": "获取菜单树结构",
                "responses": {
                    "200": {
                        "description": "菜单树结构",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.MenuResponse"
                                            }
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/monitor/cache": {
            "get": {
                "description": "获取缓存监控信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "系统监控"
                ],
                "summary": "获取缓存监控信息",
                "responses": {
                    "200": {
                        "description": "成功返回缓存监控信息",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "object"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                        }
                    }
                }
            }
        },
        "/api/admin/monitor/database": {
            "get": {
                "description": "获取数据库监控信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "系统监控"
                ],
                "summary": "获取数据库监控信息",
                "responses": {
                    "200": {
                        "description": "成功返回数据库监控信息",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "object"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                        }
                    }
                }
            }
        },
        "/api/admin/monitor/redis-keys": {
            "get": {
                "description": "查询 Redis 键列表",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "系统监控"
                ],
                "summary": "查询 Redis 键",
                "parameters": [
                    {
                        "type": "string",
                        "description": "键模式 (默认：*)",
                        "name": "pattern",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "限制数量 (默认：100)",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功返回 Redis 键列表",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "object"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                        }
                    }
                }
            }
        },
        "/api/admin/monitor/slow-queries": {
            "get": {
                "description": "查询数据库慢查询日志",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "系统监控"
                ],
                "summary": "查询慢查询日志",
                "responses": {
                    "200": {
                        "description": "成功返回慢查询日志",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "object"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                        }
                    }
                }
            }
        },
        "/api/admin/monitor/system": {
            "get": {
                "description": "获取系统资源监控信息，包括 CPU、内存、磁盘等",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "系统监控"
                ],
                "summary": "获取系统监控信息",
                "responses": {
                    "200": {
                        "description": "成功返回系统监控信息",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_infrastructure_observability_monitor.SystemStatus"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                        }
                    }
                }
            }
        },
        "/api/admin/notices": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取公告列表（支持分页和筛选）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "公告管理"
                ],
                "summary": "获取公告列表",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "page_size",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "公告类型 (1:系统公告，2:活动公告，3:普通通知，4:紧急通知)",
                        "name": "type",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "公告状态 (0:草稿，1:已发布，2:已归档)",
                        "name": "status",
                        "in": "query"
                    },
                    {
                        "type": "boolean",
                        "description": "是否置顶",
                        "name": "is_top",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "公告列表",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.NoticeListResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "更新公告信息（仅限草稿状态）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "公告管理"
                ],
                "summary": "更新公告",
                "parameters": [
                    {
                        "description": "公告信息",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.UpdateNoticeRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "更新成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.NoticeResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "创建新的公告（草稿状态）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "公告管理"
                ],
                "summary": "创建公告",
                "parameters": [
                    {
                        "description": "公告信息",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.CreateNoticeRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "创建成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.NoticeResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/notices/:id": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据 ID 获取公告详细信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "公告管理"
                ],
                "summary": "获取公告详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "公告 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "公告详情",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.NoticeResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "删除公告（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "公告管理"
                ],
                "summary": "删除公告",
                "parameters": [
                    {
                        "type": "string",
                        "description": "公告 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "删除成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/notices/:id/is-read": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "检查当前用户是否已阅读指定公告",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "公告管理"
                ],
                "summary": "检查用户是否已读公告",
                "parameters": [
                    {
                        "type": "string",
                        "description": "公告 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "是否已读",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "boolean"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/notices/:id/publish": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "将草稿状态的公告发布",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "公告管理"
                ],
                "summary": "发布公告",
                "parameters": [
                    {
                        "type": "string",
                        "description": "公告 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "发布成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/notices/:id/read": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "标记公告为已读状态，并增加浏览次数",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "公告管理"
                ],
                "summary": "标记公告已读",
                "parameters": [
                    {
                        "type": "string",
                        "description": "公告 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "标记成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/notices/statistics": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取公告的统计信息（总数、已发布、草稿、已归档）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "公告管理"
                ],
                "summary": "获取公告统计",
                "responses": {
                    "200": {
                        "description": "统计信息",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.NoticeStatisticsResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/online-user": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取当前所有在线用户信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "在线用户管理"
                ],
                "summary": "获取在线用户列表",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "page_size",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "在线用户列表",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.OnlineUserListResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/online-user/:user_id": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据用户 ID 获取在线用户详细信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "在线用户管理"
                ],
                "summary": "获取在线用户详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户 ID",
                        "name": "user_id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "在线用户详情",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.OnlineUserResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "用户不在线",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "强制指定用户下线（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "在线用户管理"
                ],
                "summary": "强制用户下线",
                "parameters": [
                    {
                        "description": "用户 ID",
                        "name": "user_id",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "integer"
                        }
                    },
                    {
                        "description": "下线原因",
                        "name": "reason",
                        "in": "body",
                        "schema": {
                            "type": "string"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "强制下线成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "用户不在线",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/online-user/batch": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "批量强制指定用户下线（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "在线用户管理"
                ],
                "summary": "批量强制用户下线",
                "parameters": [
                    {
                        "description": "批量下线请求",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.BatchForceOfflineRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "批量强制下线成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/online-user/count": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取当前在线用户总数",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "在线用户管理"
                ],
                "summary": "获取在线用户数量",
                "responses": {
                    "200": {
                        "description": "在线用户数量",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "integer",
                                            "format": "int64"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/online-user/online": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据用户 ID 检查用户是否在线",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "在线用户管理"
                ],
                "summary": "检查用户是否在线",
                "parameters": [
                    {
                        "type": "integer",
                        "format": "int64",
                        "description": "用户 ID",
                        "name": "user_id",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "是否在线",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "boolean"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/permission-logs": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取系统权限变更日志（支持分页和筛选）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "权限日志管理"
                ],
                "summary": "获取权限日志列表",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "page_size",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "format": "int64",
                        "description": "用户 ID",
                        "name": "user_id",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "用户名",
                        "name": "userName",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "操作类型",
                        "name": "operation",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "模块",
                        "name": "module",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "开始日期（YYYY-MM-DD）",
                        "name": "start_date",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "结束日期（YYYY-MM-DD）",
                        "name": "end_date",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "权限日志列表",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "allOf": [
                                                {
                                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.PageResult"
                                                },
                                                {
                                                    "type": "object",
                                                    "properties": {
                                                        "list": {
                                                            "type": "array",
                                                            "items": {}
                                                        }
                                                    }
                                                }
                                            ]
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/position": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取岗位列表（支持分页和筛选）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "岗位管理"
                ],
                "summary": "获取岗位列表",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "page_size",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "状态 (0:禁用，1:启用)",
                        "name": "status",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "岗位列表",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.PositionListResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "更新岗位信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "岗位管理"
                ],
                "summary": "更新岗位",
                "parameters": [
                    {
                        "description": "岗位信息",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.UpdatePositionRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "更新成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.PositionResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "创建新的岗位",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "岗位管理"
                ],
                "summary": "创建岗位",
                "parameters": [
                    {
                        "description": "岗位信息",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.CreatePositionRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "创建成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.PositionResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/position/:id": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据 ID 获取岗位详细信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "岗位管理"
                ],
                "summary": "获取岗位详情",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "岗位 ID",
                        "name": "id",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "岗位详情",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.PositionResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "删除岗位（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "岗位管理"
                ],
                "summary": "删除岗位",
                "parameters": [
                    {
                        "type": "string",
                        "description": "岗位 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "删除成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/position/all": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取所有岗位，用于前端选择岗位选项",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "岗位管理"
                ],
                "summary": "获取所有岗位",
                "responses": {
                    "200": {
                        "description": "岗位列表",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.PositionResponse"
                                            }
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/role": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取角色列表",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "角色管理"
                ],
                "summary": "获取角色列表",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "pageSize",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "角色名称",
                        "name": "name",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "状态(-1:全部,0:禁用,1:启用)",
                        "name": "status",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.RoleListResponse"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "更新角色信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "角色管理"
                ],
                "summary": "更新角色",
                "parameters": [
                    {
                        "description": "角色信息",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.UpdateRoleRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "创建新的角色",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "角色管理"
                ],
                "summary": "创建角色",
                "parameters": [
                    {
                        "description": "角色信息",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.CreateRoleRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.RoleResponse"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "删除角色",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "角色管理"
                ],
                "summary": "删除角色",
                "parameters": [
                    {
                        "description": "角色ID",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.DeleteRoleRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                        }
                    }
                }
            }
        },
        "/api/admin/role/:id/available-apis": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取尚未分配给该角色的 API 路由列表",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "角色管理"
                ],
                "summary": "获取角色可用的 API 列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "角色 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.APIResponse"
                                            }
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/api/admin/role/:id/available-menus": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取完整的菜单树结构，用于角色权限配置",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "角色管理"
                ],
                "summary": "获取角色可授权的菜单树",
                "parameters": [
                    {
                        "type": "string",
                        "description": "角色 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.MenuResponse"
                                            }
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/api/admin/role/:id/detail": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据路径参数 ID 获取角色详情，支持 RESTful 风格",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "角色管理"
                ],
                "summary": "根据路径参数 ID 获取角色详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "角色 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.RoleDetailResponse"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/api/admin/role/:id/permissions": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "保存角色的菜单、按钮和 API 权限",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "角色管理"
                ],
                "summary": "保存角色权限",
                "parameters": [
                    {
                        "type": "string",
                        "description": "角色 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "权限信息",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.SaveRolePermissionsRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                        }
                    }
                }
            }
        },
        "/api/admin/role/all": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取所有角色，用于前端选择角色选项",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "角色管理"
                ],
                "summary": "获取所有角色",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.RoleResponse"
                                            }
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/api/admin/role/assign-api-routes": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "分配API路由到角色",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "角色管理"
                ],
                "summary": "分配API路由到角色",
                "parameters": [
                    {
                        "description": "角色ID和API路由ID列表",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.AssignToRoleRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                        }
                    }
                }
            }
        },
        "/api/admin/role/assign-menus": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "分配菜单到角色",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "角色管理"
                ],
                "summary": "分配菜单到角色",
                "parameters": [
                    {
                        "description": "角色ID和菜单ID列表",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.AssignMenuToRoleRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                        }
                    }
                }
            }
        },
        "/api/admin/role/assign-users": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "分配用户到角色",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "角色管理"
                ],
                "summary": "分配用户到角色",
                "parameters": [
                    {
                        "description": "角色ID和用户ID列表",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.AssignUserToRoleRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                        }
                    }
                }
            }
        },
        "/api/admin/role/detail": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据 ID 获取角色详情",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "角色管理"
                ],
                "summary": "根据 ID 获取角色",
                "parameters": [
                    {
                        "type": "string",
                        "description": "角色 ID",
                        "name": "id",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.RoleDetailResponse"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/api/admin/roles/:roleId/permission-logs": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取指定角色的权限变更历史",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "权限日志管理"
                ],
                "summary": "获取角色权限日志",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "角色 ID",
                        "name": "role_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "page_size",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "角色权限日志列表",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "allOf": [
                                                {
                                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.PageResult"
                                                },
                                                {
                                                    "type": "object",
                                                    "properties": {
                                                        "list": {
                                                            "type": "array",
                                                            "items": {}
                                                        }
                                                    }
                                                }
                                            ]
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/subscriber": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取订阅者列表（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "事件管理"
                ],
                "summary": "获取订阅者列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "事件名称",
                        "name": "eventName",
                        "in": "query"
                    },
                    {
                        "type": "boolean",
                        "description": "是否活跃",
                        "name": "isActive",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "pageSize",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "订阅者列表",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.SubscriberListResponse"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "创建一个事件订阅者（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "事件管理"
                ],
                "summary": "创建订阅者",
                "parameters": [
                    {
                        "description": "创建订阅者请求参数",
                        "name": "subscriber",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.CreateSubscriberRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "订阅者创建成功",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.SubscriberResponse"
                            }
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/subscriber/:id": {
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "删除一个订阅者（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "事件管理"
                ],
                "summary": "删除订阅者",
                "parameters": [
                    {
                        "type": "string",
                        "description": "订阅者 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "订阅者删除成功",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "订阅者不存在",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/task": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取所有任务列表（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "任务管理"
                ],
                "summary": "获取所有任务",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "pageSize",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "任务ID",
                        "name": "taskId",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "任务类型",
                        "name": "taskType",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "任务列表",
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.TaskListResponse"
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "创建一个新的任务（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "任务管理"
                ],
                "summary": "创建任务",
                "parameters": [
                    {
                        "description": "创建任务请求参数",
                        "name": "task",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.AddTaskRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "任务创建成功",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.TaskResponse"
                            }
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/task/:id": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据 ID 获取单个任务详情（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "任务管理"
                ],
                "summary": "获取单个任务",
                "parameters": [
                    {
                        "type": "string",
                        "description": "任务 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "任务详情",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.TaskResponse"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "任务不存在",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "更新一个已存在的任务（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "任务管理"
                ],
                "summary": "更新任务",
                "parameters": [
                    {
                        "type": "string",
                        "description": "任务 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "更新任务请求参数",
                        "name": "task",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.AddTaskRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "任务更新成功",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.TaskResponse"
                            }
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "任务不存在",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据ID移除一个任务（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "任务管理"
                ],
                "summary": "移除任务",
                "parameters": [
                    {
                        "type": "string",
                        "description": "任务ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "任务移除成功",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/task/:id/disable": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "禁用一个已启用的任务（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "任务管理"
                ],
                "summary": "禁用任务",
                "parameters": [
                    {
                        "type": "string",
                        "description": "任务ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "任务禁用成功",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "任务不存在",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/task/:id/enable": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "启用一个已禁用的任务（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "任务管理"
                ],
                "summary": "启用任务",
                "parameters": [
                    {
                        "type": "string",
                        "description": "任务ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "任务启用成功",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "任务不存在",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/task/:id/execution-logs": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据任务 ID 查询任务执行历史记录（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "任务管理"
                ],
                "summary": "查询任务执行历史",
                "parameters": [
                    {
                        "type": "string",
                        "description": "任务 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "pageSize",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "执行日志列表",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.TaskExecutionLogsResponse"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/task/:id/group": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "设置指定任务的分组（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "任务管理"
                ],
                "summary": "设置任务分组",
                "parameters": [
                    {
                        "type": "string",
                        "description": "任务 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "分组信息",
                        "name": "task",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.SetTaskGroupRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "设置成功",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "任务不存在",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/task/:id/retry": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据ID重试一个失败的任务（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "任务管理"
                ],
                "summary": "重试任务",
                "parameters": [
                    {
                        "type": "string",
                        "description": "任务ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "任务重试触发成功",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/task/:id/start": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据ID启动一个任务（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "任务管理"
                ],
                "summary": "启动任务",
                "parameters": [
                    {
                        "type": "string",
                        "description": "任务ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "任务启动成功",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/task/:id/stop": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "根据ID停止一个任务（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "任务管理"
                ],
                "summary": "停止任务",
                "parameters": [
                    {
                        "type": "string",
                        "description": "任务ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "任务停止成功",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/task/dashboard": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取任务统计面板数据，包括任务总数、启用数、禁用数、今日执行数（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "任务管理"
                ],
                "summary": "获取任务统计面板",
                "responses": {
                    "200": {
                        "description": "统计面板数据",
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/task/execution-logs": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "支持多条件搜索任务执行日志，可按任务ID、执行结果、日期范围筛选（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "任务管理"
                ],
                "summary": "搜索任务执行日志",
                "parameters": [
                    {
                        "type": "string",
                        "description": "任务 ID",
                        "name": "taskId",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "执行结果（success/failed）",
                        "name": "result",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "开始日期（RFC3339格式）",
                        "name": "startDate",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "结束日期（RFC3339格式）",
                        "name": "endDate",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "pageSize",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "执行日志列表",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.TaskExecutionLogsResponse"
                            }
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/task/statistics": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取所有任务的执行统计信息（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "任务管理"
                ],
                "summary": "获取任务统计",
                "responses": {
                    "200": {
                        "description": "任务统计列表",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "array",
                                "items": {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.TaskStatisticsResponse"
                                }
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "无权限",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/user": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取用户列表（支持分页和筛选）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户管理"
                ],
                "summary": "获取用户列表",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "page_size",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "用户名",
                        "name": "userName",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "邮箱",
                        "name": "email",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "角色",
                        "name": "role",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "状态",
                        "name": "status",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "用户列表",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "allOf": [
                                                {
                                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.PageResult"
                                                },
                                                {
                                                    "type": "object",
                                                    "properties": {
                                                        "list": {
                                                            "type": "array",
                                                            "items": {
                                                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.User"
                                                            }
                                                        }
                                                    }
                                                }
                                            ]
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "创建新用户账户",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户管理"
                ],
                "summary": "创建用户",
                "parameters": [
                    {
                        "description": "创建用户请求参数",
                        "name": "create",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.AddUserRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "创建成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.UserResponse"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/user/get-user-settings": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取当前登录用户的系统设置",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户管理"
                ],
                "summary": "获取用户设置",
                "responses": {
                    "200": {
                        "description": "用户设置信息",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "object",
                                            "additionalProperties": {
                                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.UserSetting"
                                            }
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/user/info": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "获取当前登录用户的详细信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户管理"
                ],
                "summary": "获取用户信息",
                "responses": {
                    "200": {
                        "description": "用户信息",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.UserInfoResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "更新用户的基本信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户管理"
                ],
                "summary": "更新用户信息",
                "parameters": [
                    {
                        "description": "更新用户请求参数",
                        "name": "update_user",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.UpdateUserRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "更新成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.User"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/user/password": {
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "修改当前用户的密码",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户管理"
                ],
                "summary": "修改密码",
                "parameters": [
                    {
                        "description": "修改密码请求参数",
                        "name": "change_password",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.ChangePasswordRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "修改成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/user/reset-password": {
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "重置用户密码为默认密码123456（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户管理"
                ],
                "summary": "重置密码",
                "parameters": [
                    {
                        "description": "重置密码请求参数",
                        "name": "reset_password",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.ResetPasswordRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "重置成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/user/setUserAuthorities": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "为用户设置多个角色权限",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户管理"
                ],
                "summary": "设置用户权限（多角色）",
                "parameters": [
                    {
                        "description": "权限设置信息",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.SetUserAuthoritiesRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "设置成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/user/setUserAuthority": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "为用户设置单个角色权限",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户管理"
                ],
                "summary": "设置用户权限（单角色）",
                "parameters": [
                    {
                        "description": "权限设置信息",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.SetUserAuthorityRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "设置成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/user/switch-role": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "切换当前用户的活动角色（不修改用户角色关联关系）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户管理"
                ],
                "summary": "切换用户角色",
                "parameters": [
                    {
                        "description": "切换角色请求参数",
                        "name": "switch_role",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_request.SwitchRoleRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "切换成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/user/update-user-settings": {
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "更新当前登录用户的系统设置",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户管理"
                ],
                "summary": "更新用户设置",
                "parameters": [
                    {
                        "description": "用户设置参数",
                        "name": "user_setting",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.UserSetting"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "更新成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "object",
                                            "additionalProperties": {
                                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.UserSetting"
                                            }
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/admin/user/{id}": {
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "删除用户（管理员权限）",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户管理"
                ],
                "summary": "删除用户",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "用户ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "删除成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/payment": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "创建一笔新的支付订单",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "支付管理"
                ],
                "summary": "创建支付",
                "parameters": [
                    {
                        "description": "创建支付请求参数",
                        "name": "payment",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_wx_request.CreatePaymentRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "支付创建成功",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_wx_response.PaymentResponse"
                            }
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "未授权",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "服务器内部错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/wx/auth/login": {
            "post": {
                "description": "使用微信授权码登录获取用户信息和令牌",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "微信认证"
                ],
                "summary": "微信授权码登录",
                "parameters": [
                    {
                        "description": "微信授权码登录请求参数",
                        "name": "code",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_wx_request.WechatLoginRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "登录成功，包含用户信息和令牌",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_wx_response.WXLoginResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "认证失败",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/wx/auth/refresh-token": {
            "post": {
                "description": "使用 refresh token 获取新的 access token 和 refresh token",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "微信认证"
                ],
                "summary": "刷新令牌",
                "parameters": [
                    {
                        "description": "刷新令牌请求参数",
                        "name": "refresh",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_wx_request.WechatRefreshTokenRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "刷新成功，包含新的令牌",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_wx_response.WXLoginResponse"
                                        },
                                        "msg": {
                                            "type": "string"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "认证失败",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/gateway/services": {
            "get": {
                "description": "从网关获取所有已注册的服务实例",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "网关管理"
                ],
                "summary": "获取网关服务列表",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/internal_app_base_api.GatewayServiceInfo"
                                            }
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        }
    },
    "definitions": {
        "github_com_ix-pay_ixpay-pro_internal_domain_base_entity.API": {
            "type": "object",
            "properties": {
                "authRequired": {
                    "description": "是否需要认证",
                    "type": "boolean"
                },
                "authType": {
                    "description": "授权类型：0-不需要授权（只要登录），1-需要授权（需要角色权限）",
                    "type": "integer"
                },
                "btnPermIds": {
                    "description": "关联的按钮权限 ID 列表",
                    "type": "array",
                    "items": {
                        "type": "integer",
                        "format": "int64"
                    }
                },
                "createdAt": {
                    "description": "创建时间",
                    "type": "string"
                },
                "createdBy": {
                    "description": "创建人 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "description": {
                    "description": "描述",
                    "type": "string"
                },
                "group": {
                    "description": "路由分组",
                    "type": "string"
                },
                "id": {
                    "description": "API ID",
                    "type": "integer",
                    "format": "int64"
                },
                "menuIds": {
                    "description": "关联的菜单 ID 列表",
                    "type": "array",
                    "items": {
                        "type": "integer",
                        "format": "int64"
                    }
                },
                "method": {
                    "description": "HTTP 方法",
                    "type": "string"
                },
                "path": {
                    "description": "路由路径",
                    "type": "string"
                },
                "roleIds": {
                    "description": "关联的角色 ID 列表",
                    "type": "array",
                    "items": {
                        "type": "integer",
                        "format": "int64"
                    }
                },
                "status": {
                    "description": "状态：1-启用，0-禁用",
                    "type": "integer"
                },
                "updatedAt": {
                    "description": "更新时间",
                    "type": "string"
                },
                "updatedBy": {
                    "description": "更新人 ID",
                    "type": "integer",
                    "format": "int64"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_domain_base_entity.BtnPerm": {
            "type": "object",
            "properties": {
                "apirouteIds": {
                    "description": "关联的 API 路由 ID 列表",
                    "type": "array",
                    "items": {
                        "type": "integer",
                        "format": "int64"
                    }
                },
                "apiroutes": {
                    "description": "关联的 API 路由对象列表",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.API"
                    }
                },
                "code": {
                    "description": "权限编码，如：user:create, user:edit",
                    "type": "string"
                },
                "createdAt": {
                    "description": "创建时间",
                    "type": "string"
                },
                "createdBy": {
                    "description": "创建人 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "description": {
                    "description": "权限描述",
                    "type": "string"
                },
                "id": {
                    "description": "按钮权限 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "menu": {
                    "description": "所属菜单（新增）",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.Menu"
                        }
                    ]
                },
                "menuID": {
                    "description": "所属菜单 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "name": {
                    "description": "权限名称，如：创建用户，编辑用户",
                    "type": "string"
                },
                "roleIds": {
                    "description": "关联的角色 ID 列表",
                    "type": "array",
                    "items": {
                        "type": "integer",
                        "format": "int64"
                    }
                },
                "roles": {
                    "description": "关联的角色对象列表关联的角色列表（新增）",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.Role"
                    }
                },
                "status": {
                    "description": "状态：1-启用，0-禁用",
                    "type": "integer"
                },
                "updatedAt": {
                    "description": "更新时间",
                    "type": "string"
                },
                "updatedBy": {
                    "description": "更新人 ID",
                    "type": "integer",
                    "format": "int64"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_domain_base_entity.DailyStat": {
            "type": "object",
            "properties": {
                "date": {
                    "description": "日期",
                    "type": "string"
                },
                "failedCount": {
                    "description": "失败次数",
                    "type": "integer",
                    "format": "int64"
                },
                "successCount": {
                    "description": "成功次数",
                    "type": "integer",
                    "format": "int64"
                },
                "totalCount": {
                    "description": "总次数",
                    "type": "integer",
                    "format": "int64"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_domain_base_entity.DeleteImpact": {
            "type": "object",
            "properties": {
                "affectedApisCount": {
                    "description": "影响的 API 数量",
                    "type": "integer",
                    "format": "int64"
                },
                "affectedRolesCount": {
                    "description": "受影响的角色数量",
                    "type": "integer",
                    "format": "int64"
                },
                "btnPermsCount": {
                    "description": "按钮权限数量",
                    "type": "integer",
                    "format": "int64"
                },
                "childMenusCount": {
                    "description": "子菜单数量",
                    "type": "integer",
                    "format": "int64"
                },
                "level": {
                    "description": "影响等级：LOW, MEDIUM, HIGH",
                    "type": "string"
                },
                "warning": {
                    "description": "警告信息",
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_domain_base_entity.Department": {
            "type": "object",
            "properties": {
                "children": {
                    "description": "子部门",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.Department"
                    }
                },
                "createdAt": {
                    "description": "创建时间",
                    "type": "string"
                },
                "createdBy": {
                    "description": "创建人 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "description": {
                    "description": "部门描述",
                    "type": "string"
                },
                "id": {
                    "description": "部门 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "leader": {
                    "description": "部门负责人（新增）",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.User"
                        }
                    ]
                },
                "leaderID": {
                    "description": "部门负责人 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "name": {
                    "description": "部门名称",
                    "type": "string"
                },
                "parent": {
                    "description": "父部门（新增）",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.Department"
                        }
                    ]
                },
                "parentID": {
                    "description": "父部门 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "sort": {
                    "description": "排序",
                    "type": "integer"
                },
                "status": {
                    "description": "状态：1-正常，0-禁用",
                    "type": "integer"
                },
                "updatedAt": {
                    "description": "更新时间",
                    "type": "string"
                },
                "updatedBy": {
                    "description": "更新人 ID",
                    "type": "integer",
                    "format": "int64"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_domain_base_entity.FailedIPStat": {
            "type": "object",
            "properties": {
                "failedCount": {
                    "description": "失败次数",
                    "type": "integer",
                    "format": "int64"
                },
                "ip": {
                    "description": "IP 地址",
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_domain_base_entity.HourlyStat": {
            "type": "object",
            "properties": {
                "count": {
                    "description": "统计数量",
                    "type": "integer",
                    "format": "int64"
                },
                "hour": {
                    "description": "小时 (0-23)",
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_domain_base_entity.LoginStatistics": {
            "type": "object",
            "properties": {
                "dailyStats": {
                    "description": "每日统计",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.DailyStat"
                    }
                },
                "endTime": {
                    "description": "统计结束时间",
                    "type": "string"
                },
                "failedCount": {
                    "description": "失败登录次数",
                    "type": "integer",
                    "format": "int64"
                },
                "failedIPs": {
                    "description": "失败最多的 IP",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.FailedIPStat"
                    }
                },
                "hourlyStats": {
                    "description": "每小时统计",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.HourlyStat"
                    }
                },
                "startTime": {
                    "description": "统计开始时间",
                    "type": "string"
                },
                "successCount": {
                    "description": "成功登录次数",
                    "type": "integer",
                    "format": "int64"
                },
                "topIPs": {
                    "description": "登录最频繁 IP",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.TopIPStat"
                    }
                },
                "topUsers": {
                    "description": "登录最频繁用户",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.TopUserStat"
                    }
                },
                "totalCount": {
                    "description": "登录总次数",
                    "type": "integer",
                    "format": "int64"
                },
                "uniqueIPs": {
                    "description": "独立 IP 数",
                    "type": "integer",
                    "format": "int64"
                },
                "uniqueUsers": {
                    "description": "独立用户数",
                    "type": "integer",
                    "format": "int64"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_domain_base_entity.Menu": {
            "type": "object",
            "properties": {
                "activeMenu": {
                    "description": "当前激活的菜单",
                    "type": "string"
                },
                "affix": {
                    "description": "是否固定在标签栏",
                    "type": "boolean"
                },
                "apirouteIds": {
                    "description": "关联的 API 路由 ID 列表",
                    "type": "array",
                    "items": {
                        "type": "integer",
                        "format": "int64"
                    }
                },
                "apiroutes": {
                    "description": "关联的 API 路由列表（新增）",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.API"
                    }
                },
                "breadcrumb": {
                    "description": "是否在面包屑显示",
                    "type": "boolean"
                },
                "btnPermIds": {
                    "description": "菜单下的按钮权限 ID 列表",
                    "type": "array",
                    "items": {
                        "type": "integer",
                        "format": "int64"
                    }
                },
                "btnPerms": {
                    "description": "菜单下的按钮权限列表（新增）",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.BtnPerm"
                    }
                },
                "children": {
                    "description": "子菜单",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.Menu"
                    }
                },
                "component": {
                    "description": "组件路径",
                    "type": "string"
                },
                "createdAt": {
                    "description": "创建时间",
                    "type": "string"
                },
                "createdBy": {
                    "description": "创建人 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "defaultMenu": {
                    "description": "是否默认菜单",
                    "type": "boolean"
                },
                "frameLoading": {
                    "description": "iframe 加载动画",
                    "type": "boolean"
                },
                "frameSrc": {
                    "description": "iframe 地址（如果是 iframe 类型）",
                    "type": "string"
                },
                "hidden": {
                    "description": "是否隐藏",
                    "type": "boolean"
                },
                "icon": {
                    "description": "菜单图标",
                    "type": "string"
                },
                "id": {
                    "description": "菜单 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "isExt": {
                    "description": "是否外部链接",
                    "type": "boolean"
                },
                "keepAlive": {
                    "description": "是否缓存",
                    "type": "boolean"
                },
                "meta": {
                    "description": "菜单元数据",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.MenuMeta"
                        }
                    ]
                },
                "name": {
                    "description": "路由名称，前端组件名",
                    "type": "string"
                },
                "parent": {
                    "description": "父菜单（新增）",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.Menu"
                        }
                    ]
                },
                "parentID": {
                    "description": "父菜单 ID，0 表示顶级菜单",
                    "type": "integer",
                    "format": "int64"
                },
                "path": {
                    "description": "路由路径",
                    "type": "string"
                },
                "permission": {
                    "description": "权限标识",
                    "type": "string"
                },
                "redirect": {
                    "description": "重定向地址",
                    "type": "string"
                },
                "roleIds": {
                    "description": "关联的角色 ID 列表",
                    "type": "array",
                    "items": {
                        "type": "integer",
                        "format": "int64"
                    }
                },
                "roles": {
                    "description": "关联的角色列表（新增）",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.Role"
                    }
                },
                "sort": {
                    "description": "排序号",
                    "type": "integer"
                },
                "status": {
                    "description": "状态：0 禁用，1 启用",
                    "type": "integer"
                },
                "title": {
                    "description": "菜单标题",
                    "type": "string"
                },
                "type": {
                    "description": "菜单类型：1 目录，2 菜单，3 按钮，4iframe",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.MenuType"
                        }
                    ]
                },
                "updatedAt": {
                    "description": "更新时间",
                    "type": "string"
                },
                "updatedBy": {
                    "description": "更新人 ID",
                    "type": "integer",
                    "format": "int64"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_domain_base_entity.MenuMeta": {
            "type": "object",
            "properties": {
                "activeMenu": {
                    "description": "当前激活的菜单",
                    "type": "string"
                },
                "affix": {
                    "description": "是否固定在标签栏",
                    "type": "boolean"
                },
                "breadcrumb": {
                    "description": "是否在面包屑显示",
                    "type": "boolean"
                },
                "defaultMenu": {
                    "description": "是否默认菜单",
                    "type": "boolean"
                },
                "frameLoading": {
                    "description": "iframe 加载动画",
                    "type": "boolean"
                },
                "frameSrc": {
                    "description": "iframe 地址（如果是 iframe 类型）",
                    "type": "string"
                },
                "icon": {
                    "description": "菜单图标",
                    "type": "string"
                },
                "keepAlive": {
                    "description": "是否缓存",
                    "type": "boolean"
                },
                "title": {
                    "description": "菜单标题",
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_domain_base_entity.MenuType": {
            "type": "integer",
            "enum": [
                1,
                2,
                3,
                4
            ],
            "x-enum-comments": {
                "MenuTypeButton": "按钮",
                "MenuTypeDirectory": "目录",
                "MenuTypeIframe": "内嵌 iframe",
                "MenuTypeMenu": "菜单"
            },
            "x-enum-descriptions": [
                "目录",
                "菜单",
                "按钮",
                "内嵌 iframe"
            ],
            "x-enum-varnames": [
                "MenuTypeDirectory",
                "MenuTypeMenu",
                "MenuTypeButton",
                "MenuTypeIframe"
            ]
        },
        "github_com_ix-pay_ixpay-pro_internal_domain_base_entity.PermissionGroup": {
            "type": "object",
            "properties": {
                "apirouteIds": {
                    "description": "权限组关联的接口路由 ID 列表",
                    "type": "array",
                    "items": {
                        "type": "integer",
                        "format": "int64"
                    }
                },
                "apiroutes": {
                    "description": "权限组关联的接口路由对象列表",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.API"
                    }
                },
                "btnPermIds": {
                    "description": "权限组关联的按钮权限 ID 列表",
                    "type": "array",
                    "items": {
                        "type": "integer",
                        "format": "int64"
                    }
                },
                "btnPerms": {
                    "description": "权限组关联的按钮权限对象列表",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.BtnPerm"
                    }
                },
                "createdAt": {
                    "description": "创建时间",
                    "type": "string"
                },
                "createdBy": {
                    "description": "创建人 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "description": {
                    "description": "权限组描述",
                    "type": "string"
                },
                "id": {
                    "description": "权限组 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "name": {
                    "description": "权限组名称",
                    "type": "string"
                },
                "roleIds": {
                    "description": "权限组关联的角色 ID 列表",
                    "type": "array",
                    "items": {
                        "type": "integer",
                        "format": "int64"
                    }
                },
                "roles": {
                    "description": "权限组关联的角色对象列表",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.PermissionGroup"
                    }
                },
                "sort": {
                    "description": "排序",
                    "type": "integer"
                },
                "status": {
                    "description": "状态：1-启用，0-禁用",
                    "type": "integer"
                },
                "updatedAt": {
                    "description": "更新时间",
                    "type": "string"
                },
                "updatedBy": {
                    "description": "更新人 ID",
                    "type": "integer",
                    "format": "int64"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_domain_base_entity.Position": {
            "type": "object",
            "properties": {
                "code": {
                    "description": "岗位编码",
                    "type": "string"
                },
                "createdAt": {
                    "description": "创建时间",
                    "type": "string"
                },
                "createdBy": {
                    "description": "创建人 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "description": {
                    "description": "岗位描述",
                    "type": "string"
                },
                "id": {
                    "description": "岗位 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "name": {
                    "description": "岗位名称",
                    "type": "string"
                },
                "sort": {
                    "description": "排序",
                    "type": "integer"
                },
                "status": {
                    "description": "状态：1-正常，0-禁用",
                    "type": "integer"
                },
                "updatedAt": {
                    "description": "更新时间",
                    "type": "string"
                },
                "updatedBy": {
                    "description": "更新人 ID",
                    "type": "integer",
                    "format": "int64"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_domain_base_entity.Role": {
            "type": "object",
            "properties": {
                "apirouteIds": {
                    "description": "角色关联的接口路由 ID 列表",
                    "type": "array",
                    "items": {
                        "type": "integer",
                        "format": "int64"
                    }
                },
                "apiroutes": {
                    "description": "角色关联的接口路由对象列表",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.API"
                    }
                },
                "btnPermIds": {
                    "description": "角色关联的按钮权限 ID 列表",
                    "type": "array",
                    "items": {
                        "type": "integer",
                        "format": "int64"
                    }
                },
                "btnPerms": {
                    "description": "角色关联的按钮权限对象列表",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.BtnPerm"
                    }
                },
                "children": {
                    "description": "子角色",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.Role"
                    }
                },
                "code": {
                    "description": "角色编码",
                    "type": "string"
                },
                "createdAt": {
                    "description": "创建时间",
                    "type": "string"
                },
                "createdBy": {
                    "description": "创建人 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "description": {
                    "description": "角色描述",
                    "type": "string"
                },
                "id": {
                    "description": "角色 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "isSystem": {
                    "description": "是否系统角色",
                    "type": "boolean"
                },
                "menuIds": {
                    "description": "角色关联的菜单 ID 列表",
                    "type": "array",
                    "items": {
                        "type": "integer",
                        "format": "int64"
                    }
                },
                "menus": {
                    "description": "角色关联的菜单对象列表",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.Menu"
                    }
                },
                "name": {
                    "description": "角色名称",
                    "type": "string"
                },
                "parent": {
                    "description": "父角色",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.Role"
                        }
                    ]
                },
                "parentID": {
                    "description": "父角色 ID，支持角色继承",
                    "type": "integer",
                    "format": "int64"
                },
                "permissionGroupIds": {
                    "description": "角色关联的权限组 ID 列表",
                    "type": "array",
                    "items": {
                        "type": "integer",
                        "format": "int64"
                    }
                },
                "permissionGroups": {
                    "description": "角色关联的权限组对象列表",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.PermissionGroup"
                    }
                },
                "sort": {
                    "description": "排序",
                    "type": "integer"
                },
                "status": {
                    "description": "状态：1-启用，0-禁用",
                    "type": "integer"
                },
                "type": {
                    "description": "角色类型：1-系统角色，2-业务角色，3-数据角色",
                    "type": "integer"
                },
                "updatedAt": {
                    "description": "更新时间",
                    "type": "string"
                },
                "updatedBy": {
                    "description": "更新人 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "userIds": {
                    "description": "角色关联的用户 ID 列表",
                    "type": "array",
                    "items": {
                        "type": "integer",
                        "format": "int64"
                    }
                },
                "users": {
                    "description": "角色关联的用户对象列表",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.User"
                    }
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_domain_base_entity.TopIPStat": {
            "type": "object",
            "properties": {
                "ip": {
                    "description": "IP 地址",
                    "type": "string"
                },
                "loginCount": {
                    "description": "登录次数",
                    "type": "integer",
                    "format": "int64"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_domain_base_entity.TopUserStat": {
            "type": "object",
            "properties": {
                "loginCount": {
                    "description": "登录次数",
                    "type": "integer",
                    "format": "int64"
                },
                "userID": {
                    "description": "用户 ID",
                    "type": "string"
                },
                "username": {
                    "description": "用户名",
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_domain_base_entity.User": {
            "type": "object",
            "properties": {
                "address": {
                    "description": "地址",
                    "type": "string"
                },
                "avatar": {
                    "description": "用户头像 URL",
                    "type": "string"
                },
                "birthday": {
                    "description": "生日",
                    "type": "string"
                },
                "createdAt": {
                    "description": "创建时间",
                    "type": "string"
                },
                "createdBy": {
                    "description": "创建人 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "department": {
                    "description": "所属部门",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.Department"
                        }
                    ]
                },
                "departmentID": {
                    "description": "部门 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "email": {
                    "description": "电子邮箱",
                    "type": "string"
                },
                "entryDate": {
                    "description": "入职日期",
                    "type": "string"
                },
                "gender": {
                    "description": "性别：0-未知，1-男，2-女",
                    "type": "integer"
                },
                "id": {
                    "description": "用户 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "lastLoginIP": {
                    "description": "最后登录 IP",
                    "type": "string"
                },
                "lastLoginTime": {
                    "description": "最后登录时间",
                    "type": "string"
                },
                "nickname": {
                    "description": "用户昵称",
                    "type": "string"
                },
                "passwordHash": {
                    "description": "密码哈希值",
                    "type": "string"
                },
                "phone": {
                    "description": "手机号码",
                    "type": "string"
                },
                "position": {
                    "description": "所属岗位",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.Position"
                        }
                    ]
                },
                "positionID": {
                    "description": "岗位 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "roleIds": {
                    "description": "用户关联的角色 ID 列表",
                    "type": "array",
                    "items": {
                        "type": "integer",
                        "format": "int64"
                    }
                },
                "roles": {
                    "description": "角色列表",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_domain_base_entity.Role"
                    }
                },
                "specialBtnPermissionIds": {
                    "description": "用户特殊按钮权限 ID 列表",
                    "type": "array",
                    "items": {
                        "type": "integer",
                        "format": "int64"
                    }
                },
                "specialPermissionIds": {
                    "description": "用户特殊权限 ID 列表",
                    "type": "array",
                    "items": {
                        "type": "integer",
                        "format": "int64"
                    }
                },
                "status": {
                    "description": "用户状态，1-正常，0-禁用",
                    "type": "integer"
                },
                "updatedAt": {
                    "description": "更新时间",
                    "type": "string"
                },
                "updatedBy": {
                    "description": "更新人 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "username": {
                    "description": "用户名，唯一标识",
                    "type": "string"
                },
                "wechatOpenID": {
                    "description": "微信 OpenID，唯一",
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_domain_base_entity.UserSetting": {
            "type": "object",
            "properties": {
                "autoLogin": {
                    "description": "自动登录",
                    "type": "boolean"
                },
                "createdAt": {
                    "description": "创建时间",
                    "type": "string"
                },
                "createdBy": {
                    "description": "创建人 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "fontSize": {
                    "description": "字体大小",
                    "type": "integer"
                },
                "id": {
                    "description": "用户设置 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "language": {
                    "description": "语言",
                    "type": "string"
                },
                "navbarColor": {
                    "description": "导航栏颜色",
                    "type": "string"
                },
                "rememberPassword": {
                    "description": "记住密码",
                    "type": "boolean"
                },
                "sidebarColor": {
                    "description": "侧边栏颜色",
                    "type": "string"
                },
                "themeColor": {
                    "description": "主题颜色",
                    "type": "string"
                },
                "updatedAt": {
                    "description": "更新时间",
                    "type": "string"
                },
                "updatedBy": {
                    "description": "更新人 ID",
                    "type": "integer",
                    "format": "int64"
                },
                "userID": {
                    "description": "用户 ID",
                    "type": "integer",
                    "format": "int64"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.AddTaskRequest": {
            "type": "object",
            "required": [
                "expression",
                "params",
                "task_id",
                "task_type",
                "type"
            ],
            "properties": {
                "description": {
                    "type": "string"
                },
                "expression": {
                    "type": "string"
                },
                "group": {
                    "type": "string"
                },
                "params": {
                    "type": "array",
                    "items": {
                        "type": "integer"
                    }
                },
                "retry_count": {
                    "type": "integer",
                    "maximum": 10,
                    "minimum": 0
                },
                "task_id": {
                    "type": "string"
                },
                "task_type": {
                    "type": "string",
                    "enum": [
                        "http",
                        "database",
                        "cache",
                        "script"
                    ]
                },
                "type": {
                    "type": "string",
                    "enum": [
                        "cron",
                        "one_time"
                    ]
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.AddUserRequest": {
            "type": "object",
            "required": [
                "password",
                "userName"
            ],
            "properties": {
                "avatar": {
                    "type": "string",
                    "maxLength": 255
                },
                "departmentId": {
                    "type": "string",
                    "minimum": 1,
                    "example": "0"
                },
                "email": {
                    "type": "string"
                },
                "nickname": {
                    "type": "string",
                    "maxLength": 50
                },
                "password": {
                    "type": "string",
                    "maxLength": 30,
                    "minLength": 6
                },
                "phone": {
                    "type": "string",
                    "maxLength": 20
                },
                "positionId": {
                    "type": "string",
                    "minimum": 1,
                    "example": "0"
                },
                "roles": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "status": {
                    "type": "integer",
                    "maximum": 1,
                    "minimum": 0
                },
                "userName": {
                    "type": "string",
                    "maxLength": 50,
                    "minLength": 3
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.AssignBtnPermToRoleRequest": {
            "type": "object",
            "required": [
                "btnPermIds",
                "roleId"
            ],
            "properties": {
                "btnPermIds": {
                    "type": "array",
                    "minItems": 1,
                    "items": {
                        "type": "string"
                    }
                },
                "roleId": {
                    "type": "string",
                    "minimum": 1,
                    "example": "0"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.AssignMenuToRoleRequest": {
            "type": "object",
            "required": [
                "menuIds",
                "roleId"
            ],
            "properties": {
                "menuIds": {
                    "type": "array",
                    "minItems": 1,
                    "items": {
                        "type": "string"
                    }
                },
                "roleId": {
                    "type": "string",
                    "minimum": 1,
                    "example": "0"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.AssignToBtnPermRequest": {
            "type": "object",
            "required": [
                "btnPermId",
                "ids"
            ],
            "properties": {
                "btnPermId": {
                    "type": "string",
                    "minimum": 1,
                    "example": "0"
                },
                "ids": {
                    "type": "array",
                    "minItems": 1,
                    "items": {
                        "type": "string"
                    }
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.AssignToRoleRequest": {
            "type": "object",
            "required": [
                "ids",
                "roleId"
            ],
            "properties": {
                "ids": {
                    "type": "array",
                    "minItems": 1,
                    "items": {
                        "type": "string"
                    }
                },
                "roleId": {
                    "type": "string",
                    "minimum": 1,
                    "example": "0"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.AssignUserToRoleRequest": {
            "type": "object",
            "required": [
                "roleId",
                "userIds"
            ],
            "properties": {
                "roleId": {
                    "type": "string",
                    "minimum": 1,
                    "example": "0"
                },
                "userIds": {
                    "type": "array",
                    "minItems": 1,
                    "items": {
                        "type": "string"
                    }
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.BatchDeleteLoginLogsRequest": {
            "type": "object",
            "required": [
                "ids"
            ],
            "properties": {
                "ids": {
                    "description": "登录日志 ID 列表",
                    "type": "array",
                    "items": {
                        "type": "integer"
                    }
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.BatchDeleteOperationLogRequest": {
            "type": "object",
            "required": [
                "ids"
            ],
            "properties": {
                "ids": {
                    "type": "array",
                    "minItems": 1,
                    "items": {
                        "type": "string"
                    }
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.BatchForceOfflineRequest": {
            "type": "object",
            "required": [
                "userIds"
            ],
            "properties": {
                "reason": {
                    "description": "下线原因",
                    "type": "string"
                },
                "userIds": {
                    "description": "用户 ID 列表",
                    "type": "array",
                    "items": {
                        "type": "integer"
                    }
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.ChangePasswordRequest": {
            "type": "object",
            "required": [
                "newPassword",
                "oldPassword"
            ],
            "properties": {
                "newPassword": {
                    "type": "string",
                    "maxLength": 30,
                    "minLength": 6
                },
                "oldPassword": {
                    "type": "string",
                    "maxLength": 30,
                    "minLength": 6
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.ClearLoginLogsRequest": {
            "type": "object",
            "required": [
                "endTime",
                "startTime"
            ],
            "properties": {
                "endTime": {
                    "description": "结束时间（YYYY-MM-DD HH:mm:ss）",
                    "type": "string"
                },
                "startTime": {
                    "description": "开始时间（YYYY-MM-DD HH:mm:ss）",
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.ClearOperationLogByTimeRangeRequest": {
            "type": "object",
            "required": [
                "endTime",
                "startTime"
            ],
            "properties": {
                "endTime": {
                    "type": "string"
                },
                "startTime": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.CreateAPIRequest": {
            "type": "object",
            "required": [
                "method",
                "path"
            ],
            "properties": {
                "authRequired": {
                    "type": "boolean"
                },
                "authType": {
                    "type": "integer",
                    "enum": [
                        0,
                        1
                    ]
                },
                "btnPermIds": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "description": {
                    "type": "string",
                    "maxLength": 500
                },
                "group": {
                    "type": "string",
                    "maxLength": 100
                },
                "menuIds": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "method": {
                    "type": "string",
                    "enum": [
                        "GET",
                        "POST",
                        "PUT",
                        "DELETE",
                        "PATCH",
                        "HEAD",
                        "OPTIONS"
                    ]
                },
                "path": {
                    "type": "string",
                    "maxLength": 255
                },
                "roleIds": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "status": {
                    "type": "integer",
                    "enum": [
                        0,
                        1
                    ]
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.CreateBtnPermRequest": {
            "type": "object",
            "required": [
                "code",
                "menuId",
                "name"
            ],
            "properties": {
                "code": {
                    "type": "string",
                    "maxLength": 100,
                    "minLength": 2
                },
                "description": {
                    "type": "string",
                    "maxLength": 255
                },
                "menuId": {
                    "type": "string",
                    "minimum": 1,
                    "example": "0"
                },
                "name": {
                    "type": "string",
                    "maxLength": 50,
                    "minLength": 2
                },
                "status": {
                    "type": "integer",
                    "enum": [
                        0,
                        1
                    ]
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.CreateConfigRequest": {
            "type": "object",
            "required": [
                "configKey"
            ],
            "properties": {
                "configKey": {
                    "description": "配置键",
                    "type": "string",
                    "maxLength": 100
                },
                "configType": {
                    "description": "配置类型",
                    "type": "string",
                    "maxLength": 20
                },
                "configValue": {
                    "description": "配置值",
                    "type": "string"
                },
                "description": {
                    "description": "描述",
                    "type": "string",
                    "maxLength": 255
                },
                "status": {
                    "description": "状态：1-启用 0-禁用",
                    "type": "integer",
                    "enum": [
                        0,
                        1
                    ]
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.CreateDepartmentRequest": {
            "type": "object",
            "required": [
                "name"
            ],
            "properties": {
                "description": {
                    "description": "部门描述",
                    "type": "string"
                },
                "leaderId": {
                    "description": "部门负责人 ID",
                    "type": "string",
                    "example": "0"
                },
                "name": {
                    "description": "部门名称",
                    "type": "string"
                },
                "parentId": {
                    "description": "父部门 ID，默认为 0（顶级部门）",
                    "type": "string",
                    "example": "0"
                },
                "sort": {
                    "description": "排序，默认为 0",
                    "type": "integer"
                },
                "status": {
                    "description": "状态：1-正常，0-禁用，默认为 1",
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.CreateDictItemRequest": {
            "type": "object",
            "required": [
                "dictId",
                "itemKey",
                "itemValue"
            ],
            "properties": {
                "description": {
                    "description": "描述",
                    "type": "string",
                    "maxLength": 255
                },
                "dictId": {
                    "description": "字典 ID",
                    "type": "string",
                    "example": "0"
                },
                "itemKey": {
                    "description": "字典项键",
                    "type": "string",
                    "maxLength": 50
                },
                "itemValue": {
                    "description": "字典项值",
                    "type": "string",
                    "maxLength": 255
                },
                "sort": {
                    "description": "排序",
                    "type": "integer",
                    "minimum": 0
                },
                "status": {
                    "description": "状态：1-启用 0-禁用",
                    "type": "integer",
                    "enum": [
                        0,
                        1
                    ]
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.CreateDictRequest": {
            "type": "object",
            "required": [
                "dictCode",
                "dictName"
            ],
            "properties": {
                "description": {
                    "description": "描述",
                    "type": "string",
                    "maxLength": 255
                },
                "dictCode": {
                    "description": "字典编码",
                    "type": "string",
                    "maxLength": 50
                },
                "dictName": {
                    "description": "字典名称",
                    "type": "string",
                    "maxLength": 100
                },
                "status": {
                    "description": "状态：1-启用 0-禁用",
                    "type": "integer",
                    "enum": [
                        0,
                        1
                    ]
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.CreateEventRequest": {
            "type": "object",
            "required": [
                "name"
            ],
            "properties": {
                "delay": {
                    "description": "延迟秒数",
                    "type": "integer"
                },
                "maxRetries": {
                    "type": "integer"
                },
                "metadata": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "payload": {
                    "type": "string"
                },
                "priority": {
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.CreateNoticeRequest": {
            "type": "object",
            "required": [
                "content",
                "title",
                "type"
            ],
            "properties": {
                "content": {
                    "description": "公告内容",
                    "type": "string"
                },
                "description": {
                    "description": "公告描述/摘要",
                    "type": "string"
                },
                "isTop": {
                    "description": "是否置顶",
                    "type": "boolean"
                },
                "sort": {
                    "description": "排序",
                    "type": "integer"
                },
                "title": {
                    "description": "公告标题",
                    "type": "string"
                },
                "type": {
                    "description": "公告类型：1-系统公告，2-活动公告，3-普通通知，4-紧急通知",
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.CreatePositionRequest": {
            "type": "object",
            "required": [
                "code",
                "name"
            ],
            "properties": {
                "code": {
                    "description": "岗位编码",
                    "type": "string"
                },
                "description": {
                    "description": "岗位描述",
                    "type": "string"
                },
                "name": {
                    "description": "岗位名称",
                    "type": "string"
                },
                "sort": {
                    "description": "排序，默认为 0",
                    "type": "integer"
                },
                "status": {
                    "description": "状态：1-正常，0-禁用，默认为 1",
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.CreateRoleRequest": {
            "type": "object",
            "required": [
                "name"
            ],
            "properties": {
                "description": {
                    "type": "string",
                    "maxLength": 255
                },
                "name": {
                    "type": "string",
                    "maxLength": 50,
                    "minLength": 2
                },
                "status": {
                    "type": "integer",
                    "enum": [
                        0,
                        1
                    ]
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.CreateSubscriberRequest": {
            "type": "object",
            "required": [
                "eventName",
                "name"
            ],
            "properties": {
                "endpoint": {
                    "type": "string"
                },
                "eventName": {
                    "type": "string"
                },
                "maxRetries": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "priority": {
                    "type": "integer"
                },
                "timeout": {
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.DeleteBtnPermRequest": {
            "type": "object",
            "required": [
                "id"
            ],
            "properties": {
                "id": {
                    "type": "string",
                    "minimum": 1,
                    "example": "0"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.DeleteRoleRequest": {
            "type": "object",
            "required": [
                "id"
            ],
            "properties": {
                "id": {
                    "type": "string",
                    "example": "0"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.JsonInBlacklistRequest": {
            "type": "object",
            "required": [
                "token"
            ],
            "properties": {
                "token": {
                    "description": "JWT token",
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.LoginRequest": {
            "type": "object",
            "required": [
                "password",
                "userName"
            ],
            "properties": {
                "captcha": {
                    "type": "string"
                },
                "captchaId": {
                    "type": "string"
                },
                "password": {
                    "type": "string"
                },
                "userName": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.RecordLoginRequest": {
            "type": "object",
            "required": [
                "ip",
                "userId",
                "userName"
            ],
            "properties": {
                "browser": {
                    "description": "浏览器信息",
                    "type": "string"
                },
                "device": {
                    "description": "设备信息",
                    "type": "string"
                },
                "errorMsg": {
                    "description": "错误信息",
                    "type": "string"
                },
                "ip": {
                    "description": "登录 IP",
                    "type": "string"
                },
                "os": {
                    "description": "操作系统",
                    "type": "string"
                },
                "place": {
                    "description": "登录地点",
                    "type": "string"
                },
                "success": {
                    "description": "是否成功",
                    "type": "boolean"
                },
                "userAgent": {
                    "description": "原始 User-Agent",
                    "type": "string"
                },
                "userId": {
                    "description": "用户 ID",
                    "type": "string",
                    "example": "0"
                },
                "userName": {
                    "description": "用户名",
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.RegisterRequest": {
            "type": "object",
            "required": [
                "email",
                "password",
                "userName"
            ],
            "properties": {
                "email": {
                    "type": "string"
                },
                "password": {
                    "type": "string",
                    "maxLength": 30,
                    "minLength": 6
                },
                "userName": {
                    "type": "string",
                    "maxLength": 50,
                    "minLength": 3
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.ResetPasswordRequest": {
            "type": "object",
            "required": [
                "userId"
            ],
            "properties": {
                "userId": {
                    "type": "string",
                    "example": "0"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.RevokeBtnPermFromRoleRequest": {
            "type": "object",
            "required": [
                "btnPermId",
                "roleId"
            ],
            "properties": {
                "btnPermId": {
                    "type": "string",
                    "minimum": 1,
                    "example": "0"
                },
                "roleId": {
                    "type": "string",
                    "minimum": 1,
                    "example": "0"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.RevokeFromBtnPermRequest": {
            "type": "object",
            "required": [
                "btnPermId",
                "id"
            ],
            "properties": {
                "btnPermId": {
                    "type": "string",
                    "minimum": 1,
                    "example": "0"
                },
                "id": {
                    "type": "string",
                    "minimum": 1,
                    "example": "0"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.SaveRolePermissionsRequest": {
            "type": "object",
            "required": [
                "apiRouteIds",
                "btnPermIds",
                "menuIds"
            ],
            "properties": {
                "apiRouteIds": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "btnPermIds": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "menuIds": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.SetTaskGroupRequest": {
            "type": "object",
            "required": [
                "group"
            ],
            "properties": {
                "group": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.SetUserAuthoritiesRequest": {
            "type": "object",
            "required": [
                "roleIds",
                "userId"
            ],
            "properties": {
                "roleIds": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "userId": {
                    "description": "支持 string 和 int64"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.SetUserAuthorityRequest": {
            "type": "object",
            "required": [
                "roleId",
                "userId"
            ],
            "properties": {
                "roleId": {
                    "type": "string"
                },
                "userId": {
                    "description": "支持 string 和 int64"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.SwitchRoleRequest": {
            "type": "object",
            "required": [
                "roleId"
            ],
            "properties": {
                "roleId": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.UpdateAPIRequest": {
            "type": "object",
            "required": [
                "id",
                "method",
                "path"
            ],
            "properties": {
                "authRequired": {
                    "type": "boolean"
                },
                "authType": {
                    "type": "integer",
                    "enum": [
                        0,
                        1
                    ]
                },
                "btnPermIds": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "description": {
                    "type": "string",
                    "maxLength": 500
                },
                "group": {
                    "type": "string",
                    "maxLength": 100
                },
                "id": {
                    "type": "string",
                    "example": "0"
                },
                "menuIds": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "method": {
                    "type": "string",
                    "enum": [
                        "GET",
                        "POST",
                        "PUT",
                        "DELETE",
                        "PATCH",
                        "HEAD",
                        "OPTIONS"
                    ]
                },
                "path": {
                    "type": "string",
                    "maxLength": 255
                },
                "roleIds": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "status": {
                    "type": "integer",
                    "enum": [
                        0,
                        1
                    ]
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.UpdateBtnPermRequest": {
            "type": "object",
            "required": [
                "code",
                "id",
                "menuId",
                "name"
            ],
            "properties": {
                "code": {
                    "type": "string",
                    "maxLength": 100,
                    "minLength": 2
                },
                "description": {
                    "type": "string",
                    "maxLength": 255
                },
                "id": {
                    "type": "string",
                    "minimum": 1,
                    "example": "0"
                },
                "menuId": {
                    "type": "string",
                    "minimum": 1,
                    "example": "0"
                },
                "name": {
                    "type": "string",
                    "maxLength": 50,
                    "minLength": 2
                },
                "status": {
                    "type": "integer",
                    "enum": [
                        0,
                        1
                    ]
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.UpdateConfigRequest": {
            "type": "object",
            "required": [
                "configKey",
                "id"
            ],
            "properties": {
                "configKey": {
                    "description": "配置键",
                    "type": "string",
                    "maxLength": 100
                },
                "configType": {
                    "description": "配置类型",
                    "type": "string",
                    "maxLength": 20
                },
                "configValue": {
                    "description": "配置值",
                    "type": "string"
                },
                "description": {
                    "description": "描述",
                    "type": "string",
                    "maxLength": 255
                },
                "id": {
                    "description": "配置 ID",
                    "type": "string",
                    "example": "0"
                },
                "status": {
                    "description": "状态：1-启用 0-禁用",
                    "type": "integer",
                    "enum": [
                        0,
                        1
                    ]
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.UpdateDepartmentLeaderRequest": {
            "type": "object",
            "required": [
                "leaderId"
            ],
            "properties": {
                "leaderId": {
                    "description": "部门负责人 ID",
                    "type": "string",
                    "example": "0"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.UpdateDepartmentRequest": {
            "type": "object",
            "required": [
                "id",
                "name"
            ],
            "properties": {
                "description": {
                    "description": "部门描述",
                    "type": "string"
                },
                "id": {
                    "description": "部门 ID",
                    "type": "string",
                    "example": "0"
                },
                "leaderId": {
                    "description": "部门负责人 ID",
                    "type": "string",
                    "example": "0"
                },
                "name": {
                    "description": "部门名称",
                    "type": "string"
                },
                "parentId": {
                    "description": "父部门 ID",
                    "type": "string",
                    "example": "0"
                },
                "sort": {
                    "description": "排序",
                    "type": "integer"
                },
                "status": {
                    "description": "状态：1-正常，0-禁用",
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.UpdateDictItemRequest": {
            "type": "object",
            "required": [
                "dictId",
                "id",
                "itemKey",
                "itemValue"
            ],
            "properties": {
                "description": {
                    "description": "描述",
                    "type": "string",
                    "maxLength": 255
                },
                "dictId": {
                    "description": "字典 ID",
                    "type": "string",
                    "example": "0"
                },
                "id": {
                    "description": "字典项 ID",
                    "type": "string",
                    "example": "0"
                },
                "itemKey": {
                    "description": "字典项键",
                    "type": "string",
                    "maxLength": 50
                },
                "itemValue": {
                    "description": "字典项值",
                    "type": "string",
                    "maxLength": 255
                },
                "sort": {
                    "description": "排序",
                    "type": "integer",
                    "minimum": 0
                },
                "status": {
                    "description": "状态：1-启用 0-禁用",
                    "type": "integer",
                    "enum": [
                        0,
                        1
                    ]
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.UpdateDictRequest": {
            "type": "object",
            "required": [
                "dictCode",
                "dictName",
                "id"
            ],
            "properties": {
                "description": {
                    "description": "描述",
                    "type": "string",
                    "maxLength": 255
                },
                "dictCode": {
                    "description": "字典编码",
                    "type": "string",
                    "maxLength": 50
                },
                "dictName": {
                    "description": "字典名称",
                    "type": "string",
                    "maxLength": 100
                },
                "id": {
                    "description": "字典 ID",
                    "type": "string",
                    "example": "0"
                },
                "status": {
                    "description": "状态：1-启用 0-禁用",
                    "type": "integer",
                    "enum": [
                        0,
                        1
                    ]
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.UpdateNoticeRequest": {
            "type": "object",
            "required": [
                "content",
                "id",
                "title",
                "type"
            ],
            "properties": {
                "content": {
                    "description": "公告内容",
                    "type": "string"
                },
                "description": {
                    "description": "公告描述/摘要",
                    "type": "string"
                },
                "id": {
                    "description": "公告 ID",
                    "type": "string",
                    "example": "0"
                },
                "isTop": {
                    "description": "是否置顶",
                    "type": "boolean"
                },
                "sort": {
                    "description": "排序",
                    "type": "integer"
                },
                "title": {
                    "description": "公告标题",
                    "type": "string"
                },
                "type": {
                    "description": "公告类型：1-系统公告，2-活动公告，3-普通通知，4-紧急通知",
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.UpdatePositionRequest": {
            "type": "object",
            "required": [
                "code",
                "id",
                "name"
            ],
            "properties": {
                "code": {
                    "description": "岗位编码",
                    "type": "string"
                },
                "description": {
                    "description": "岗位描述",
                    "type": "string"
                },
                "id": {
                    "description": "岗位 ID",
                    "type": "string",
                    "example": "0"
                },
                "name": {
                    "description": "岗位名称",
                    "type": "string"
                },
                "sort": {
                    "description": "排序",
                    "type": "integer"
                },
                "status": {
                    "description": "状态：1-正常，0-禁用",
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.UpdateRoleRequest": {
            "type": "object",
            "required": [
                "id",
                "name"
            ],
            "properties": {
                "description": {
                    "type": "string",
                    "maxLength": 255
                },
                "id": {
                    "type": "string",
                    "example": "0"
                },
                "name": {
                    "type": "string",
                    "maxLength": 50,
                    "minLength": 2
                },
                "status": {
                    "type": "integer",
                    "enum": [
                        0,
                        1
                    ]
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_request.UpdateUserRequest": {
            "type": "object",
            "required": [
                "id"
            ],
            "properties": {
                "avatar": {
                    "type": "string",
                    "maxLength": 255
                },
                "departmentId": {
                    "type": "string",
                    "example": "0"
                },
                "email": {
                    "type": "string"
                },
                "id": {
                    "type": "string"
                },
                "nickname": {
                    "type": "string",
                    "maxLength": 50
                },
                "phone": {
                    "type": "string",
                    "maxLength": 20
                },
                "positionId": {
                    "type": "string",
                    "example": "0"
                },
                "roles": {
                    "description": "角色列表，支持多选",
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "status": {
                    "description": "1: active, 0: inactive",
                    "type": "integer",
                    "maximum": 1,
                    "minimum": 0
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.APIResponse": {
            "type": "object",
            "properties": {
                "authRequired": {
                    "type": "boolean"
                },
                "authType": {
                    "type": "integer"
                },
                "btnPermIds": {
                    "type": "array",
                    "items": {
                        "type": "integer"
                    }
                },
                "createdAt": {
                    "type": "string"
                },
                "createdBy": {
                    "type": "string",
                    "example": "0"
                },
                "description": {
                    "type": "string"
                },
                "group": {
                    "type": "string"
                },
                "id": {
                    "type": "string",
                    "example": "0"
                },
                "menuIds": {
                    "type": "array",
                    "items": {
                        "type": "integer"
                    }
                },
                "method": {
                    "type": "string"
                },
                "path": {
                    "type": "string"
                },
                "roleIds": {
                    "type": "array",
                    "items": {
                        "type": "integer"
                    }
                },
                "status": {
                    "type": "integer"
                },
                "updatedAt": {
                    "type": "string"
                },
                "updatedBy": {
                    "type": "string",
                    "example": "0"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.AbnormalLoginInfoDTO": {
            "type": "object",
            "properties": {
                "failedCount": {
                    "type": "integer"
                },
                "ip": {
                    "type": "string"
                },
                "lastFailedTime": {
                    "type": "string"
                },
                "riskDescription": {
                    "type": "string"
                },
                "riskLevel": {
                    "type": "string"
                },
                "usernames": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.AuthorityInfo": {
            "type": "object",
            "properties": {
                "defaultRouter": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.BtnPermForRole": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "string",
                    "example": "0"
                },
                "isAssigned": {
                    "type": "boolean"
                },
                "menuId": {
                    "type": "string",
                    "example": "0"
                },
                "name": {
                    "type": "string"
                },
                "status": {
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.BtnPermResponse": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "string"
                },
                "createdAt": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "string",
                    "example": "0"
                },
                "menuId": {
                    "type": "string",
                    "example": "0"
                },
                "name": {
                    "type": "string"
                },
                "status": {
                    "type": "integer"
                },
                "updatedAt": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.CaptchaResponse": {
            "type": "object",
            "properties": {
                "captchaId": {
                    "type": "string"
                },
                "captchaLength": {
                    "type": "integer"
                },
                "openCaptcha": {
                    "type": "boolean"
                },
                "picPath": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.ConfigListResponse": {
            "type": "object",
            "properties": {
                "list": {
                    "description": "配置列表",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.ConfigResponse"
                    }
                },
                "page": {
                    "type": "integer"
                },
                "pageSize": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.ConfigResponse": {
            "type": "object",
            "properties": {
                "configKey": {
                    "description": "配置键",
                    "type": "string"
                },
                "configType": {
                    "description": "配置类型",
                    "type": "string"
                },
                "configValue": {
                    "description": "配置值",
                    "type": "string"
                },
                "createdAt": {
                    "description": "创建时间",
                    "type": "string"
                },
                "description": {
                    "description": "描述",
                    "type": "string"
                },
                "id": {
                    "description": "配置 ID",
                    "type": "string",
                    "example": "0"
                },
                "status": {
                    "description": "状态：1-启用 0-禁用",
                    "type": "integer"
                },
                "updatedAt": {
                    "description": "更新时间",
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.DeadLetterListResponse": {
            "type": "object",
            "properties": {
                "list": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.DeadLetterResponse"
                    }
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.DeadLetterResponse": {
            "type": "object",
            "properties": {
                "createdAt": {
                    "type": "string"
                },
                "eventName": {
                    "type": "string"
                },
                "id": {
                    "type": "string",
                    "example": "0"
                },
                "maxRetries": {
                    "type": "integer"
                },
                "reason": {
                    "type": "string"
                },
                "retryCount": {
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.DepartmentListResponse": {
            "type": "object",
            "properties": {
                "list": {
                    "description": "部门列表",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.DepartmentResponse"
                    }
                },
                "page": {
                    "type": "integer"
                },
                "pageSize": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.DepartmentResponse": {
            "type": "object",
            "properties": {
                "children": {
                    "description": "可选字段：子部门列表",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.DepartmentResponse"
                    }
                },
                "code": {
                    "description": "部门编码",
                    "type": "string"
                },
                "createdAt": {
                    "description": "创建时间",
                    "type": "string"
                },
                "email": {
                    "description": "邮箱",
                    "type": "string"
                },
                "id": {
                    "description": "部门 ID",
                    "type": "string",
                    "example": "0"
                },
                "leader": {
                    "description": "负责人",
                    "type": "string"
                },
                "name": {
                    "description": "部门名称",
                    "type": "string"
                },
                "parentId": {
                    "description": "父部门 ID",
                    "type": "string",
                    "example": "0"
                },
                "phone": {
                    "description": "联系电话",
                    "type": "string"
                },
                "sort": {
                    "description": "排序",
                    "type": "integer"
                },
                "status": {
                    "description": "状态：1-启用 0-禁用",
                    "type": "integer"
                },
                "updatedAt": {
                    "description": "更新时间",
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.DictItemListResponse": {
            "type": "object",
            "properties": {
                "list": {
                    "description": "字典项列表",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.DictItemResponse"
                    }
                },
                "page": {
                    "type": "integer"
                },
                "pageSize": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.DictItemResponse": {
            "type": "object",
            "properties": {
                "createdAt": {
                    "description": "创建时间",
                    "type": "string"
                },
                "description": {
                    "description": "描述",
                    "type": "string"
                },
                "dictId": {
                    "description": "字典 ID",
                    "type": "string",
                    "example": "0"
                },
                "id": {
                    "description": "字典项 ID",
                    "type": "string",
                    "example": "0"
                },
                "itemKey": {
                    "description": "字典项键",
                    "type": "string"
                },
                "itemValue": {
                    "description": "字典项值",
                    "type": "string"
                },
                "sort": {
                    "description": "排序",
                    "type": "integer"
                },
                "status": {
                    "description": "状态：1-启用 0-禁用",
                    "type": "integer"
                },
                "updatedAt": {
                    "description": "更新时间",
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.DictListResponse": {
            "type": "object",
            "properties": {
                "list": {
                    "description": "字典列表",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.DictResponse"
                    }
                },
                "page": {
                    "type": "integer"
                },
                "pageSize": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.DictResponse": {
            "type": "object",
            "properties": {
                "createdAt": {
                    "description": "创建时间",
                    "type": "string"
                },
                "description": {
                    "description": "描述",
                    "type": "string"
                },
                "dictCode": {
                    "description": "字典编码",
                    "type": "string"
                },
                "dictItems": {
                    "description": "字典项列表",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.DictItemResponse"
                    }
                },
                "dictName": {
                    "description": "字典名称",
                    "type": "string"
                },
                "id": {
                    "description": "字典 ID",
                    "type": "string",
                    "example": "0"
                },
                "itemCount": {
                    "description": "明细数量",
                    "type": "integer"
                },
                "status": {
                    "description": "状态：1-启用 0-禁用",
                    "type": "integer"
                },
                "updatedAt": {
                    "description": "更新时间",
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.EventListResponse": {
            "type": "object",
            "properties": {
                "list": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.EventResponse"
                    }
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.EventResponse": {
            "type": "object",
            "properties": {
                "createdAt": {
                    "type": "string"
                },
                "delaySeconds": {
                    "type": "integer"
                },
                "errorMessage": {
                    "type": "string"
                },
                "id": {
                    "type": "string",
                    "example": "0"
                },
                "maxRetries": {
                    "type": "integer"
                },
                "metadata": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "nextRetryAt": {
                    "type": "string"
                },
                "payload": {
                    "type": "string"
                },
                "priority": {
                    "type": "integer"
                },
                "processedAt": {
                    "type": "string"
                },
                "retryCount": {
                    "type": "integer"
                },
                "scheduledAt": {
                    "type": "string"
                },
                "status": {
                    "type": "integer"
                },
                "statusLabel": {
                    "type": "string"
                },
                "subscriberCount": {
                    "type": "integer"
                },
                "updatedAt": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.LoginLogDetailDTO": {
            "type": "object",
            "properties": {
                "browser": {
                    "type": "string"
                },
                "createdAt": {
                    "type": "string"
                },
                "device": {
                    "type": "string"
                },
                "id": {
                    "type": "string",
                    "example": "0"
                },
                "ip": {
                    "type": "string"
                },
                "os": {
                    "type": "string"
                },
                "place": {
                    "type": "string"
                },
                "result": {
                    "type": "integer"
                },
                "userId": {
                    "type": "string",
                    "example": "0"
                },
                "userName": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.LoginLogDetailResponse": {
            "type": "object",
            "properties": {
                "data": {
                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.LoginLogDetailDTO"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.LoginLogListDTO": {
            "type": "object",
            "properties": {
                "createdAt": {
                    "type": "string"
                },
                "id": {
                    "type": "string",
                    "example": "0"
                },
                "ip": {
                    "type": "string"
                },
                "place": {
                    "type": "string"
                },
                "result": {
                    "type": "integer"
                },
                "userName": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.LoginLogListResponse": {
            "type": "object",
            "properties": {
                "list": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.LoginLogListDTO"
                    }
                },
                "pageResult": {
                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.PageResult"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.LoginResponse": {
            "type": "object",
            "properties": {
                "accessToken": {
                    "type": "string"
                },
                "refreshToken": {
                    "type": "string"
                },
                "user": {
                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.UserInfoResponse"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.MenuInfo": {
            "type": "object",
            "properties": {
                "id": {
                    "type": "string",
                    "example": "0"
                },
                "name": {
                    "type": "string"
                },
                "parentId": {
                    "type": "string",
                    "example": "0"
                },
                "path": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.MenuMetaResp": {
            "type": "object",
            "properties": {
                "activeMenu": {
                    "description": "当前激活的菜单（驼峰命名）",
                    "type": "string"
                },
                "affix": {
                    "description": "是否固定在标签栏（驼峰命名）",
                    "type": "boolean"
                },
                "breadcrumb": {
                    "description": "是否在面包屑显示（驼峰命名）",
                    "type": "boolean"
                },
                "defaultMenu": {
                    "description": "是否默认菜单（驼峰命名）",
                    "type": "boolean"
                },
                "frameLoading": {
                    "description": "iframe 加载动画（驼峰命名）",
                    "type": "boolean"
                },
                "icon": {
                    "description": "菜单图标",
                    "type": "string"
                },
                "keepAlive": {
                    "description": "是否缓存（驼峰命名）",
                    "type": "boolean"
                },
                "title": {
                    "description": "菜单标题",
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.MenuResponse": {
            "type": "object",
            "properties": {
                "activeMenu": {
                    "description": "当前激活的菜单",
                    "type": "string"
                },
                "affix": {
                    "description": "是否固定在标签栏",
                    "type": "boolean"
                },
                "breadcrumb": {
                    "description": "是否在面包屑显示",
                    "type": "boolean"
                },
                "children": {
                    "description": "子菜单",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.MenuResponse"
                    }
                },
                "component": {
                    "description": "组件路径",
                    "type": "string"
                },
                "defaultMenu": {
                    "description": "是否默认菜单",
                    "type": "boolean"
                },
                "frameLoading": {
                    "description": "iframe 加载动画",
                    "type": "boolean"
                },
                "hidden": {
                    "description": "是否隐藏",
                    "type": "boolean"
                },
                "icon": {
                    "description": "菜单图标",
                    "type": "string"
                },
                "id": {
                    "description": "菜单 ID",
                    "type": "string",
                    "example": "0"
                },
                "isExt": {
                    "description": "是否外部链接",
                    "type": "boolean"
                },
                "keepAlive": {
                    "description": "是否缓存",
                    "type": "boolean"
                },
                "meta": {
                    "description": "菜单元数据",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.MenuMetaResp"
                        }
                    ]
                },
                "name": {
                    "description": "路由名称",
                    "type": "string"
                },
                "parentId": {
                    "description": "父菜单 ID",
                    "type": "string",
                    "example": "0"
                },
                "path": {
                    "description": "路由路径",
                    "type": "string"
                },
                "permission": {
                    "description": "权限标识",
                    "type": "string"
                },
                "redirect": {
                    "description": "重定向地址",
                    "type": "string"
                },
                "sort": {
                    "description": "排序号",
                    "type": "integer"
                },
                "status": {
                    "description": "状态：0 禁用，1 启用",
                    "type": "integer"
                },
                "title": {
                    "description": "菜单标题",
                    "type": "string"
                },
                "type": {
                    "description": "菜单类型：1 目录，2 菜单，3 按钮，4iframe",
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.NoticeListResponse": {
            "type": "object",
            "properties": {
                "list": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.NoticeResponse"
                    }
                },
                "page": {
                    "type": "integer"
                },
                "pageSize": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.NoticeResponse": {
            "type": "object",
            "properties": {
                "content": {
                    "type": "string"
                },
                "createdAt": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "string",
                    "example": "0"
                },
                "isTop": {
                    "type": "boolean"
                },
                "publishTime": {
                    "type": "string"
                },
                "publisherId": {
                    "type": "string",
                    "example": "0"
                },
                "sort": {
                    "type": "integer"
                },
                "status": {
                    "type": "integer"
                },
                "title": {
                    "type": "string"
                },
                "type": {
                    "type": "integer"
                },
                "updatedAt": {
                    "type": "string"
                },
                "viewCount": {
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.NoticeStatisticsResponse": {
            "type": "object",
            "properties": {
                "archivedCount": {
                    "type": "integer"
                },
                "draftCount": {
                    "type": "integer"
                },
                "publishedCount": {
                    "type": "integer"
                },
                "totalCount": {
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.OnlineUserListResponse": {
            "type": "object",
            "properties": {
                "list": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.OnlineUserResponse"
                    }
                },
                "page": {
                    "type": "integer"
                },
                "pageSize": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.OnlineUserResponse": {
            "type": "object",
            "properties": {
                "browser": {
                    "type": "string"
                },
                "device": {
                    "type": "string"
                },
                "lastActiveAt": {
                    "type": "string"
                },
                "loginIp": {
                    "type": "string"
                },
                "loginPlace": {
                    "type": "string"
                },
                "loginTime": {
                    "type": "string"
                },
                "nickname": {
                    "type": "string"
                },
                "os": {
                    "type": "string"
                },
                "sessionId": {
                    "type": "string"
                },
                "userAgent": {
                    "type": "string"
                },
                "userId": {
                    "type": "string",
                    "example": "0"
                },
                "userName": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.OperationLogListResponse": {
            "type": "object",
            "properties": {
                "list": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.OperationLogResponse"
                    }
                },
                "page": {
                    "type": "integer"
                },
                "pageSize": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.OperationLogResponse": {
            "type": "object",
            "properties": {
                "clientIp": {
                    "type": "string"
                },
                "createdAt": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "duration": {
                    "type": "integer"
                },
                "errorMessage": {
                    "type": "string"
                },
                "id": {
                    "type": "string",
                    "example": "0"
                },
                "isSuccess": {
                    "type": "boolean"
                },
                "method": {
                    "type": "string"
                },
                "module": {
                    "type": "string"
                },
                "nickname": {
                    "type": "string"
                },
                "operationType": {
                    "type": "integer"
                },
                "params": {
                    "type": "string"
                },
                "path": {
                    "type": "string"
                },
                "result": {
                    "type": "string"
                },
                "statusCode": {
                    "type": "integer"
                },
                "updatedAt": {
                    "type": "string"
                },
                "userAgent": {
                    "type": "string"
                },
                "userId": {
                    "type": "string",
                    "example": "0"
                },
                "userName": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.OperationLogStatisticsResponse": {
            "type": "object",
            "properties": {
                "avgDuration": {
                    "type": "integer"
                },
                "failedCount": {
                    "type": "integer"
                },
                "operationTypes": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "properties": {
                            "count": {
                                "type": "integer"
                            },
                            "type": {
                                "type": "integer"
                            }
                        }
                    }
                },
                "successCount": {
                    "type": "integer"
                },
                "successRate": {
                    "type": "integer"
                },
                "totalCount": {
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.PositionListResponse": {
            "type": "object",
            "properties": {
                "list": {
                    "description": "岗位列表",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.PositionResponse"
                    }
                },
                "page": {
                    "type": "integer"
                },
                "pageSize": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.PositionResponse": {
            "type": "object",
            "properties": {
                "code": {
                    "description": "岗位编码",
                    "type": "string"
                },
                "createdAt": {
                    "description": "创建时间",
                    "type": "string"
                },
                "description": {
                    "description": "描述",
                    "type": "string"
                },
                "id": {
                    "description": "岗位 ID",
                    "type": "string",
                    "example": "0"
                },
                "name": {
                    "description": "岗位名称",
                    "type": "string"
                },
                "sort": {
                    "description": "排序",
                    "type": "integer"
                },
                "status": {
                    "description": "状态：1-启用 0-禁用",
                    "type": "integer"
                },
                "updatedAt": {
                    "description": "更新时间",
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.RoleDTO": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "string"
                },
                "id": {
                    "type": "string",
                    "example": "0"
                },
                "name": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.RoleDetailResponse": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "string"
                },
                "createdAt": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "string",
                    "example": "0"
                },
                "isSystem": {
                    "type": "boolean"
                },
                "menus": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.MenuInfo"
                    }
                },
                "name": {
                    "type": "string"
                },
                "parentId": {
                    "type": "string",
                    "example": "0"
                },
                "routes": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.RouteInfo"
                    }
                },
                "sort": {
                    "type": "integer"
                },
                "status": {
                    "type": "integer"
                },
                "type": {
                    "type": "integer"
                },
                "updatedAt": {
                    "type": "string"
                },
                "users": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.UserInfo"
                    }
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.RoleInfo": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "string",
                    "example": "0"
                },
                "isSystem": {
                    "type": "boolean"
                },
                "name": {
                    "type": "string"
                },
                "parentId": {
                    "type": "string",
                    "example": "0"
                },
                "sort": {
                    "type": "integer"
                },
                "status": {
                    "type": "integer"
                },
                "type": {
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.RoleListResponse": {
            "type": "object",
            "properties": {
                "list": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.RoleResponse"
                    }
                },
                "page": {
                    "type": "integer"
                },
                "pageSize": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.RoleResponse": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "string"
                },
                "createdAt": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "string",
                    "example": "0"
                },
                "isSystem": {
                    "type": "boolean"
                },
                "name": {
                    "type": "string"
                },
                "parentId": {
                    "type": "string",
                    "example": "0"
                },
                "sort": {
                    "type": "integer"
                },
                "status": {
                    "type": "integer"
                },
                "type": {
                    "type": "integer"
                },
                "updatedAt": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.RouteInfo": {
            "type": "object",
            "properties": {
                "group": {
                    "type": "string"
                },
                "id": {
                    "type": "string",
                    "example": "0"
                },
                "method": {
                    "type": "string"
                },
                "path": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.SubscriberListResponse": {
            "type": "object",
            "properties": {
                "list": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.SubscriberResponse"
                    }
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.SubscriberResponse": {
            "type": "object",
            "properties": {
                "createdAt": {
                    "type": "string"
                },
                "endpoint": {
                    "type": "string"
                },
                "eventName": {
                    "type": "string"
                },
                "failureCount": {
                    "type": "integer"
                },
                "id": {
                    "type": "string",
                    "example": "0"
                },
                "isActive": {
                    "type": "boolean"
                },
                "lastDeliveredAt": {
                    "type": "string"
                },
                "maxRetries": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "priority": {
                    "type": "integer"
                },
                "timeout": {
                    "type": "integer"
                },
                "updatedAt": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.TaskExecutionLogResponse": {
            "type": "object",
            "properties": {
                "cronExpr": {
                    "type": "string"
                },
                "duration": {
                    "type": "integer"
                },
                "errorInfo": {
                    "type": "string"
                },
                "executeAt": {
                    "type": "string"
                },
                "group": {
                    "type": "string"
                },
                "id": {
                    "type": "string",
                    "example": "0"
                },
                "operatorId": {
                    "type": "string",
                    "example": "0"
                },
                "result": {
                    "type": "string"
                },
                "retryCount": {
                    "type": "integer"
                },
                "taskId": {
                    "type": "string",
                    "example": "0"
                },
                "taskName": {
                    "type": "string"
                },
                "triggerType": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.TaskExecutionLogsResponse": {
            "type": "object",
            "properties": {
                "logs": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.TaskExecutionLogResponse"
                    }
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.TaskListResponse": {
            "type": "object",
            "properties": {
                "list": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.TaskResponse"
                    }
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.TaskResponse": {
            "type": "object",
            "properties": {
                "concurrency": {
                    "type": "string"
                },
                "createdAt": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "expression": {
                    "type": "string"
                },
                "group": {
                    "type": "string"
                },
                "id": {
                    "type": "string",
                    "example": "0"
                },
                "lastRunAt": {
                    "type": "string"
                },
                "maxRetries": {
                    "type": "integer"
                },
                "nextRunAt": {
                    "type": "string"
                },
                "retryCount": {
                    "type": "integer"
                },
                "status": {
                    "type": "string"
                },
                "statusLabel": {
                    "type": "string"
                },
                "taskId": {
                    "type": "string"
                },
                "taskType": {
                    "type": "string"
                },
                "timeout": {
                    "type": "integer"
                },
                "type": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.TaskStatisticsResponse": {
            "type": "object",
            "properties": {
                "avgDuration": {
                    "type": "number"
                },
                "failedCount": {
                    "type": "integer"
                },
                "group": {
                    "type": "string"
                },
                "lastExecuteAt": {
                    "type": "string"
                },
                "nextExecuteAt": {
                    "type": "string"
                },
                "successCount": {
                    "type": "integer"
                },
                "successRate": {
                    "type": "number"
                },
                "taskId": {
                    "type": "string",
                    "example": "0"
                },
                "taskName": {
                    "type": "string"
                },
                "totalExecutes": {
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.UserInfo": {
            "type": "object",
            "properties": {
                "id": {
                    "type": "string",
                    "example": "0"
                },
                "nickname": {
                    "type": "string"
                },
                "userName": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.UserInfoResponse": {
            "type": "object",
            "properties": {
                "authority": {
                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.AuthorityInfo"
                },
                "avatar": {
                    "type": "string"
                },
                "currentRoleId": {
                    "type": "string",
                    "example": "0"
                },
                "email": {
                    "type": "string"
                },
                "id": {
                    "type": "string",
                    "example": "0"
                },
                "nickname": {
                    "type": "string"
                },
                "phone": {
                    "type": "string"
                },
                "role": {
                    "type": "string"
                },
                "roles": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.RoleInfo"
                    }
                },
                "status": {
                    "type": "integer"
                },
                "userName": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_base_response.UserResponse": {
            "type": "object",
            "properties": {
                "avatar": {
                    "type": "string"
                },
                "createdAt": {
                    "type": "string"
                },
                "departmentId": {
                    "type": "string",
                    "example": "0"
                },
                "email": {
                    "type": "string"
                },
                "id": {
                    "type": "string",
                    "example": "0"
                },
                "nickname": {
                    "type": "string"
                },
                "phone": {
                    "type": "string"
                },
                "positionId": {
                    "type": "string",
                    "example": "0"
                },
                "roles": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_base_response.RoleDTO"
                    }
                },
                "status": {
                    "type": "integer"
                },
                "updatedAt": {
                    "type": "string"
                },
                "userName": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_wx_request.CreatePaymentRequest": {
            "type": "object",
            "required": [
                "amount",
                "currency",
                "order_id",
                "payment_method"
            ],
            "properties": {
                "amount": {
                    "type": "number"
                },
                "currency": {
                    "type": "string",
                    "enum": [
                        "CNY",
                        "USD"
                    ]
                },
                "description": {
                    "type": "string"
                },
                "order_id": {
                    "type": "string"
                },
                "payment_method": {
                    "type": "string",
                    "enum": [
                        "wechat_alipay"
                    ]
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_wx_request.WechatLoginRequest": {
            "type": "object",
            "required": [
                "code"
            ],
            "properties": {
                "code": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_wx_request.WechatRefreshTokenRequest": {
            "type": "object",
            "required": [
                "refresh_token"
            ],
            "properties": {
                "refresh_token": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_wx_response.PaymentResponse": {
            "type": "object",
            "properties": {
                "amount": {
                    "type": "number"
                },
                "created_at": {
                    "type": "string"
                },
                "currency": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "string"
                },
                "order_id": {
                    "type": "string"
                },
                "payment_method": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                },
                "transaction_id": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "user_id": {
                    "type": "string"
                },
                "wechat_pay_params": {
                    "description": "微信支付相关参数",
                    "type": "object",
                    "additionalProperties": true
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_wx_response.WXLoginResponse": {
            "type": "object",
            "properties": {
                "accessExpire": {
                    "type": "integer"
                },
                "accessToken": {
                    "type": "string"
                },
                "refreshToken": {
                    "type": "string"
                },
                "user": {
                    "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_dto_wx_response.WXUserInfoResponse"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_dto_wx_response.WXUserInfoResponse": {
            "type": "object",
            "properties": {
                "avatar": {
                    "type": "string"
                },
                "email": {
                    "type": "string"
                },
                "id": {
                    "type": "string"
                },
                "nickname": {
                    "type": "string"
                },
                "phone": {
                    "type": "string"
                },
                "role": {
                    "type": "string"
                },
                "status": {
                    "type": "integer"
                },
                "userName": {
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_infrastructure_observability_monitor.CPUStatus": {
            "type": "object",
            "properties": {
                "cores": {
                    "description": "CPU 核心数",
                    "type": "integer"
                },
                "frequency": {
                    "description": "CPU 频率（MHz）",
                    "type": "number"
                },
                "per_cpu_usage": {
                    "description": "每个 CPU 核心的使用率",
                    "type": "array",
                    "items": {
                        "type": "number"
                    }
                },
                "usage_percent": {
                    "description": "CPU 使用率百分比",
                    "type": "number"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_infrastructure_observability_monitor.DiskStatus": {
            "type": "object",
            "properties": {
                "device": {
                    "description": "设备名称",
                    "type": "string"
                },
                "free": {
                    "description": "空闲磁盘空间（字节）",
                    "type": "integer"
                },
                "fstype": {
                    "description": "文件系统类型",
                    "type": "string"
                },
                "mountpoint": {
                    "description": "挂载点",
                    "type": "string"
                },
                "total": {
                    "description": "总磁盘空间（字节）",
                    "type": "integer"
                },
                "usage_percent": {
                    "description": "磁盘使用率百分比",
                    "type": "number"
                },
                "used": {
                    "description": "已使用磁盘空间（字节）",
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_infrastructure_observability_monitor.LoadStatus": {
            "type": "object",
            "properties": {
                "cpu_percent": {
                    "description": "当前 CPU 使用率（Windows 时有效）",
                    "type": "number"
                },
                "is_simulated": {
                    "description": "是否为模拟值（Windows）",
                    "type": "boolean"
                },
                "load1": {
                    "description": "1 分钟平均负载（或 CPU 使用率）",
                    "type": "number"
                },
                "load15": {
                    "description": "15 分钟平均负载（或 CPU 使用率）",
                    "type": "number"
                },
                "load5": {
                    "description": "5 分钟平均负载（或 CPU 使用率）",
                    "type": "number"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_infrastructure_observability_monitor.MemoryStatus": {
            "type": "object",
            "properties": {
                "available": {
                    "description": "可用内存（字节）",
                    "type": "integer"
                },
                "buffers": {
                    "description": "缓冲区内存（字节）",
                    "type": "integer"
                },
                "cached": {
                    "description": "缓存内存（字节）",
                    "type": "integer"
                },
                "free": {
                    "description": "空闲内存（字节）",
                    "type": "integer"
                },
                "swap_free": {
                    "description": "Swap 空闲（字节）",
                    "type": "integer"
                },
                "swap_total": {
                    "description": "Swap 总空间（字节）",
                    "type": "integer"
                },
                "swap_used": {
                    "description": "Swap 已使用（字节）",
                    "type": "integer"
                },
                "total": {
                    "description": "总内存（字节）",
                    "type": "integer"
                },
                "usage_percent": {
                    "description": "内存使用率百分比",
                    "type": "number"
                },
                "used": {
                    "description": "已使用内存（字节）",
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_infrastructure_observability_monitor.NetworkStatus": {
            "type": "object",
            "properties": {
                "bytes_recv": {
                    "description": "接收字节数",
                    "type": "integer"
                },
                "bytes_sent": {
                    "description": "发送字节数",
                    "type": "integer"
                },
                "dropin": {
                    "description": "接收丢弃数",
                    "type": "integer"
                },
                "dropout": {
                    "description": "发送丢弃数",
                    "type": "integer"
                },
                "errin": {
                    "description": "接收错误数",
                    "type": "integer"
                },
                "errout": {
                    "description": "发送错误数",
                    "type": "integer"
                },
                "packets_recv": {
                    "description": "接收数据包数",
                    "type": "integer"
                },
                "packets_sent": {
                    "description": "发送数据包数",
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_infrastructure_observability_monitor.OSInfo": {
            "type": "object",
            "properties": {
                "hostname": {
                    "description": "主机名",
                    "type": "string"
                },
                "kernel": {
                    "description": "内核版本",
                    "type": "string"
                },
                "os": {
                    "description": "操作系统类型：windows/linux/darwin",
                    "type": "string"
                },
                "platform": {
                    "description": "平台名称",
                    "type": "string"
                },
                "uptime": {
                    "description": "系统运行时间（秒）",
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_infrastructure_observability_monitor.ProcessInfo": {
            "type": "object",
            "properties": {
                "cpu_percent": {
                    "description": "CPU 使用率",
                    "type": "number"
                },
                "mem_percent": {
                    "description": "内存使用率",
                    "type": "number"
                },
                "name": {
                    "description": "进程名称",
                    "type": "string"
                },
                "pid": {
                    "description": "进程 ID",
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_infrastructure_observability_monitor.ProcessStatus": {
            "type": "object",
            "properties": {
                "top_cpu": {
                    "description": "CPU 使用率最高的进程",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_infrastructure_observability_monitor.ProcessInfo"
                    }
                },
                "top_memory": {
                    "description": "内存使用率最高的进程",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_infrastructure_observability_monitor.ProcessInfo"
                    }
                },
                "total": {
                    "description": "进程总数",
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_infrastructure_observability_monitor.SystemStatus": {
            "type": "object",
            "properties": {
                "cpu": {
                    "description": "CPU 使用状态",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_infrastructure_observability_monitor.CPUStatus"
                        }
                    ]
                },
                "disk": {
                    "description": "磁盘使用状态（支持多磁盘）",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_infrastructure_observability_monitor.DiskStatus"
                    }
                },
                "load": {
                    "description": "系统负载状态",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_infrastructure_observability_monitor.LoadStatus"
                        }
                    ]
                },
                "memory": {
                    "description": "内存使用状态",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_infrastructure_observability_monitor.MemoryStatus"
                        }
                    ]
                },
                "network": {
                    "description": "网络使用状态",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_infrastructure_observability_monitor.NetworkStatus"
                        }
                    ]
                },
                "os_info": {
                    "description": "操作系统信息",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_infrastructure_observability_monitor.OSInfo"
                        }
                    ]
                },
                "processes": {
                    "description": "进程信息",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_ix-pay_ixpay-pro_internal_infrastructure_observability_monitor.ProcessStatus"
                        }
                    ]
                },
                "timestamp": {
                    "description": "采集时间",
                    "type": "string"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.PageResult": {
            "type": "object",
            "properties": {
                "list": {},
                "page": {
                    "type": "integer"
                },
                "pageSize": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "github_com_ix-pay_ixpay-pro_internal_utils_common_baseRes.Response": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "integer"
                },
                "data": {},
                "msg": {
                    "type": "string"
                }
            }
        },
        "internal_app_base_api.GatewayServiceInfo": {
            "type": "object",
            "properties": {
                "activeConnections": {
                    "type": "integer"
                },
                "address": {
                    "type": "string"
                },
                "id": {
                    "type": "string"
                },
                "lastSeen": {
                    "type": "string"
                },
                "metadata": {
                    "type": "object",
                    "additionalProperties": {
                        "type": "string"
                    }
                },
                "name": {
                    "type": "string"
                },
                "port": {
                    "type": "integer"
                },
                "status": {
                    "type": "string"
                }
            }
        }
    },
    "securityDefinitions": {
        "BearerAuth": {
            "type": "apiKey",
            "name": "Authorization",
            "in": "header [Bearer ]"
        }
    }
}`

// SwaggerInfo holds exported Swagger Info so clients can modify it
var SwaggerInfo = &swag.Spec{
	Version:          "1.0",
	Host:             "127.0.0.1:8081",
	BasePath:         "/",
	Schemes:          []string{},
	Title:            "微信支付API服务",
	Description:      "基于Gin和PostgreSQL的微信支付API服务",
	InfoInstanceName: "swagger",
	SwaggerTemplate:  docTemplate,
	LeftDelim:        "{{",
	RightDelim:       "}}",
}

func init() {
	swag.Register(SwaggerInfo.InstanceName(), SwaggerInfo)
}
