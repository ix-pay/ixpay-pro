...
func (c *DepartmentController) GetDepartmentList(ctx *gin.Context) {
	...
	// 获取所有部门数据
	allDepts, err := c.service.GetDepartmentTree()
	if err != nil {
	...
	// 构建部门树
	tree := c.buildDepartmentTree(allDepts, 0)

	// 转换为响应结构
	responses := make([]*response.DepartmentResponse, 0, len(tree))
	for _, dept := range tree {
		resp := c.convertToDepartmentResponseWithChildren(dept)
		responses = append(responses, &resp)
	}

	baseRes.OkWithDetailed(responses, "获取部门列表成功", ctx)
}

// convertToDepartmentResponseWithChildren 将 entity.Department 转换为 response.DepartmentResponse（包含子部门）
func (c *DepartmentController) convertToDepartmentResponseWithChildren(dept *entity.Department) response.DepartmentResponse {
	...
	if len(dept.Children) > 0 {
		children := make([]*response.DepartmentResponse, 0, len(dept.Children))
		for _, child := range dept.Children {
			childResp := c.convertToDepartmentResponseWithChildren(child)
...
func (c *DepartmentController) buildDepartmentTree(allDepts []*entity.Department, parentID int64) []*entity.Department {
	tree := make([]*entity.Department, 0)
	for _, dept := range allDepts {
		if dept.ParentID == parentID {
			// 递归构建子部门树
			dept.Children = c.buildDepartmentTree(allDepts, dept.ID)
