...
func (c *DictController) GetDictList(ctx *gin.Context) {
	...
	for _, dict := range dicts {
		itemCount := 0
		if dict.DictItems != nil {
			itemCount = len(dict.DictItems)
		}
...
// GetActiveDictItemsByCode 根据字典编码获取启用的字典项
//
//	@Summary		根据字典编码获取启用的字典项
//	@Description	根据字典编码获取该字典下所有启用的字典项，供前端下拉使用
//	@Tags			字典管理
//	@Accept			json
//	@Produce		json
//	@Security		BearerAuth
//	@Param			code	path		string														true	"字典编码"
//	@Success		200		{object}	baseRes.Response{data=[]response.DictItemResponse,msg=string}	"字典项列表"
//	@Failure		400		{object}	map[string]string											"请求参数错误"
//	@Failure		401		{object}	map[string]string											"未授权"
//	@Failure		500		{object}	map[string]string											"服务器内部错误"
//	@Router			/api/admin/dict/code/{code}/active-items [get]
func (c *DictController) GetActiveDictItemsByCode(ctx *gin.Context) {
	dictCode := ctx.Param("code")
	if dictCode == "" {
		c.log.Error("字典编码不能为空")
		baseRes.FailWithMessage("字典编码不能为空", ctx)
		return
	}

	dictItems, err := c.dictItemService.GetActiveDictItemsByCode(dictCode)
	if err != nil {
		baseRes.FailWithMessage(err.Error(), ctx)
		return
	}
	...
	baseRes.OkWithDetailed(dictItemResponses, "获取字典项成功", ctx)
}
