...
func (a *AppBase) setupRoutes() {
	...
	admin := a.router.Group("/api/admin")
	{
		...
		authenticated.Use(middleware.PermissionMiddleware(a.permissionService, a.roleRepo, a.apiRepo, a.logger, a.cache))
		{
			// 认证相关路由（需要认证）
			...
			task := authenticated.Group("/task")
			{
				...
				task.POST("/:id/enable", a.taskController.EnableTask)
				task.POST("/:id/disable", a.taskController.DisableTask)
				task.GET("", a.taskController.GetTasks)
				task.GET("/dashboard", a.taskController.GetDashboard)
				task.GET("/execution-logs", a.taskController.SearchExecutionLogs)
			...
			{
				// 获取API路由列表（支持分页和搜索）
				apis.GET("", a.apiController.GetAPIs)
				// 根据ID获取API路由
			...
			dict := authenticated.Group("/dict")
			{
				...
				dict.GET("/code/:code/active-items", a.dictController.GetActiveDictItemsByCode)
			...
			// 节点管理路由
			node := authenticated.Group("/nodes")
			{
				node.GET("", a.nodeController.GetNodeList)
			...
			// 网关服务管理路由
			gateway := authenticated.Group("/gateway")
			{
				gateway.GET("/services", a.gatewayServiceController.GetGatewayServices)
			}
		}
	}
}
