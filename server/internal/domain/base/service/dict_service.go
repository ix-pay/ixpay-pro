...
func (s *DictItemService) GetActiveDictItemsByCode(dictCode string) ([]*entity.DictItem, error) {
	// 根据编码获取字典
	dict, err := s.dictRepo.GetByCode(dictCode)
	if err != nil {
		s.log.Error("获取字典失败", "dict_code", dictCode, "error", err)
		return nil, errors.New("字典不存在")
	}
	...
	// 获取启用的字典项
	dictItems, err := s.repo.GetActiveByDictID(dict.ID)
	if err != nil {
		s.log.Error("获取启用的字典项列表失败", "dict_code", dictCode, "error", err)
		return nil, errors.New("获取启用的字典项列表失败")
	}
	return dictItems, nil
}
