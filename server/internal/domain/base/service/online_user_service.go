...
func NewOnlineUserService(repo repo.OnlineUserRepository, cache cache.Cache, log logger.Logger) *OnlineUserService {
...
// ForceOffline 强制用户下线
// 该方法会移除用户的在线状态，并将 JWT token 加入黑名单使其失效
func (s *OnlineUserService) ForceOffline(userID int64, operatorID string) error {
	...
	// 将 JWT token 加入黑名单，使 token 失效
	// 黑名单 key 格式：blacklist:user:{userId}
	blacklistKey := fmt.Sprintf("blacklist:user:%d", userID)
	expiration := 24 * time.Hour
	if user != nil && !user.LastActiveAt.IsZero() {
		expiration = time.Until(user.LastActiveAt.Add(24 * time.Hour))
		if expiration <= 0 {
			expiration = 24 * time.Hour
		}
	}
	if err := s.cache.Set(blacklistKey, time.Now().Unix(), expiration); err != nil {
		s.log.Error("添加 JWT 黑名单失败", "error", err, "user_id", userID)
		return err
	}
