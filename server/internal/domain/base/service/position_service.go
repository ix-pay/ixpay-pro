...
// CreatePosition 创建岗位
func (s *PositionService) CreatePosition(name, code, description string, createdBy int64, sort, status int) (*entity.Position, error) {
	// 参数验证
	if name == "" {
	...
	if code == "" {
		return nil, errors.New("岗位编码不能为空")
	}
	...
	// 检查岗位编码是否已存在
	existingCode, err := s.repo.GetByCode(code)
	if err == nil && existingCode != nil {
		s.log.Error("岗位编码已存在", "code", code)
...
// UpdatePosition 更新岗位
func (s *PositionService) UpdatePosition(id int64, name, code, description string, updatedBy int64, sort, status int) (*entity.Position, error) {
	// 获取岗位
	...
	if code != position.Code {
		existingCode, err := s.repo.GetByCode(code)
		if err == nil && existingCode != nil {
			s.log.Error("岗位编码已存在", "code", code, "existing_id", existingCode.ID)
			return nil, errors.New("岗位编码已存在")
		}
	}
