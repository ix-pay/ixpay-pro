...
// SearchExecutionLogs 多条件搜索任务执行日志
func (s *TaskExecutionLogService) SearchExecutionLogs(page, pageSize int, taskID *int64, result string, startDate, endDate string) ([]*entity.TaskExecutionLog, int64, error) {
	if page <= 0 {
		page = 1
	}
	if pageSize <= 0 {
	...
	logs, total, err := s.repo.Search(page, pageSize, taskID, result, startDate, endDate)
	if err != nil {
		s.log.Error("搜索任务执行日志失败", "error", err)
		return nil, 0, errors.New("搜索任务执行日志失败")
	}
