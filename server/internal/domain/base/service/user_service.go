...
type UserService struct {
	...
	onlineUserService     *OnlineUserService         // 在线用户服务
}
...
// 返回:
// - *UserService: 用户服务实现
func NewUserService(repo repo.UserRepository, settingRepo repo.UserSettingRepository, roleService *RoleService, rolePermissionService *RolePermissionService, jwtAuth *auth.JWTAuth, config *config.Config, log logger.Logger, cache cache.Cache, captcha *captcha.Captcha, loginLogService *LoginLogService, onlineUserService *OnlineUserService) *UserService {
	// 创建并返回用户服务实例，注入所有依赖
	return &UserService{
		repo:                  repo,
...
func (s *UserService) Login(userName, password, captchaId, captchaVal, ip, userAgent string) (*entity.User, string, string, time.Time, time.Time, error) {
	...
	// 【修改 2】登录成功后，将用户添加到在线用户列表
	// 解析 JWT 中的 SessionID（此处使用 accessToken 作为 SessionID）
	sessionID := accessToken
	s.onlineUserService.AddOnlineUser(
		user.ID,
		userName,
		nickname,
		sessionID,
