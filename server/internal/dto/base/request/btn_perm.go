...
type CreateBtnPermRequest struct {
	MenuID      int64  `json:"menuId,string" binding:"required,gte=1"`
...
type UpdateBtnPermRequest struct {
	ID          int64  `json:"id,string" binding:"required,gte=1"`
	MenuID      int64  `json:"menuId,string" binding:"required,gte=1"`
	Code        string `json:"code" binding:"required,min=2,max=100"`
	Name        string `json:"name" binding:"required,min=2,max=50"`
	Description string `json:"description" binding:"max=255"`
...
type AssignToBtnPermRequest struct {
	BtnPermID int64    `json:"btnPermId,string" binding:"required,gte=1"`
...
type AssignBtnPermToRoleRequest struct {
	RoleID     int64    `json:"roleId,string" binding:"required,gte=1"`
