...
type CreateDepartmentRequest struct {
	...
	ParentID    int64  `json:"parentId,string"`         // 父部门 ID，默认为 0（顶级部门）
	LeaderID    int64  `json:"leaderId,string"`         // 部门负责人 ID
...
type UpdateDepartmentRequest struct {
	ID          int64  `json:"id,string" binding:"required"` // 部门 ID
	Name        string `json:"name" binding:"required"`      // 部门名称
...
type UpdateDepartmentLeaderRequest struct {
	LeaderID int64 `json:"leaderId,string" binding:"required"` // 部门负责人 ID
}
