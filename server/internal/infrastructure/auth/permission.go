...
func (p *PermissionManager) CachePermissions(permissions []Permission) error {
	...
	// 缓存权限列表，设置过期时间为24小时
	if err := p.redisClient.SetTry("permissions:list", permissionsData, 24*time.Hour); err != nil {
	...
	for _, perm := range permissions {
		...
		if err := p.redisClient.SetTry(key, permData, 24*time.Hour); err != nil {
...
func (p *PermissionManager) GetPermission(method, path string) (*Permission, error) {
	...
	// 从缓存获取权限信息
	permData, err := p.redisClient.GetTry(key)
	if err != nil {
