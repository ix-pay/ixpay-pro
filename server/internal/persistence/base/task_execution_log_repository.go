...
// Search 多条件搜索任务执行日志
func (r *taskExecutionLogRepository) Search(page, pageSize int, taskID *int64, result string, startDate, endDate string) ([]*entity.TaskExecutionLog, int64, error) {
	var total int64
	var dbModels []taskExecutionLogModel
	...
	if startDate != "" {
		query = query.Where("execute_at >= ?", startDate)
	}
	if endDate != "" {
		query = query.Where("execute_at <= ?", endDate)
	}
