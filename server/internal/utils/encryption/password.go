...
func decodeHash(encodedHash string) (p *Argon2Params, salt, hash []byte, err error) {
	...
	if !strings.HasPrefix(encodedHash, argon2Prefix) {
		return nil, nil, nil, errors.New("哈希格式无效")
	}
	...
	for _, param := range paramParts {
		...
		switch key {
		case "m":
			_, err := fmt.Sscanf(value, "%d", &p.Memory)
			if err != nil {
				return nil, nil, nil, errors.New("内存参数无效")
			}
		case "t":
			_, err := fmt.Sscanf(value, "%d", &p.Iterations)
			if err != nil {
				return nil, nil, nil, errors.New("迭代次数参数无效")
			}
		case "p":
			var parallelism uint32
			_, err := fmt.Sscanf(value, "%d", &parallelism)
			if err != nil {
				return nil, nil, nil, errors.New("并行度参数无效")
			}
	...
	if err != nil {
		return nil, nil, nil, errors.New("盐值格式无效")
	}
