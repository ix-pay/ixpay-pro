...
func TestMenuService_DeleteImpactLevels(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.affectedRolesCount >= 3 {
				expectedLevel = "HIGH"
			} else if tc.affectedRolesCount > 0 || tc.childMenusCount > 0 {
				expectedLevel = "MEDIUM"
			} else {
				expectedLevel = "LOW"
			}
...
func TestMenuService_DeleteImpactCalculation(t *testing.T) {
	...
	// 验证总影响范围
	totalImpact := impact.ChildMenusCount + impact.AffectedRolesCount + impact.AffectedApisCount
	assert.Equal(t, int64(13), totalImpact, "总影响范围应正确")
}
...
func TestMenuService_DeleteImpactEdgeCases(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// 验证高影响判断
			isHighImpact := tc.affectedRolesCount >= 3
...
func TestMenuService_DeleteImpactWarningMessages(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// 验证警告信息
			hasWarning := impact.ChildMenusCount > 0 || impact.AffectedRolesCount > 0
