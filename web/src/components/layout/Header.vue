        ...
        <template #dropdown>
          <el-dropdown-menu>
            <el-dropdown-item
              ...
              :key="role.id"
              :class="{ 'is-active': String(role.id) === String(currentRoleId) }"
  ...
  try {
    ...
    if (res.code === 0) {
      ...
      if (userStore.userInfo) {
        userStore.userInfo.role = currentRoleName.value
      }
      // 刷新页面以应用新角色权限
      setTimeout(() => {
        location.reload()
      ...
      ElMessage.error(res.msg || '角色切换失败')
    }
  } catch (error) {
...
const initUserRoles = () => {
  ...
  // 设置当前角色
  if (roles.length > 0) {
    // 优先使用后端返回的 currentRoleId，如果没有则使用第一个角色的 id
